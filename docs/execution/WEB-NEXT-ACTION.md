...
Next task:
WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT-v1.240

Objective:
Continue the sequential Real Browser UI/UX Layout pass for the public release-trust-and-checksum guide after v1.239 `/guides/support-and-community-guide` closure. Keep the work page-scoped, Base First and browser-evidence driven.

Current FE scope: select `/guides/release-trust-and-checksum-guide` as the next single active page after v1.239 `/guides/support-and-community-guide` closure. Complete `/guides/release-trust-and-checksum-guide` fully before any other page. Use its registered public route target as the comparison guardrail and prioritize real release-trust-and-checksum guide UI/UX Layout: clear first steps, full source-authored descriptions, useful orientation and existing reading routes, honest gameplay boundaries, spacing, typography, responsive density, keyboard/focus and actual interaction. No invented guide content, live wiki, account personalization, search backend, game controls or readiness claims.
...
4. Do not move past `/guides/release-trust-and-checksum-guide` until closure evidence exists:
...
6. If execution drifts, return to `/guides/release-trust-and-checksum-guide`, inspect rendered layout, check Base First owners, modify the shared base where reusable, and continue until `/guides/release-trust-and-checksum-guide` is closed.
