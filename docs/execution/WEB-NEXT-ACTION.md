...
Current FE scope: inspect and improve route-depth continuity across Public/Portal/Ops after v1.41. Focus on deep routes that still feel disconnected, text-heavy, or visually weak; check navigation paths, visual anchors, typography caps, horizontal overflow, mobile readability and fixture boundary clarity. Prefer shared `packages/ui` ownership for reusable layout/components. Use selected existing game-art derivatives only when they improve design review and record provenance.

Lifecycle: SELECT → SPEC_LOCK → IMPLEMENT FE-only route-depth improvements → SOURCE_VERIFY → RUNTIME_VERIFY → VISUAL_REVIEW → HANDOFF → CLOSED.

Required evidence: source validator for the selected route-depth task, app/UI typecheck, relevant production build, Playwright desktop/mobile e2e with navigation/font-size/layout/image/overflow assertions, screenshot review. No independent backend, no duplicate DTO owners, no fake fetch, no forms, no enabling fixture mutation controls.
...
Historical continuity: WEB-PUBLIC-ACCESSIBILITY-READABILITY-POLISH-v1.15 remains historical evidence. WEB-01 through WEB-07 environment-limited claims are not globally upgraded by scoped v1.36–v1.41 tests; rerun relevant package/runtime/browser gates before claiming them fully closed.
