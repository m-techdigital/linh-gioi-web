...
```text
WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT-v1.146
```
...
Base UI/UX Layout is mandatory and takes precedence before page-local UI/CSS: before adding page-local layout or style, search shared owners and extend `packages/design-tokens` or `packages/ui` for reusable patterns. App-local duplicates of reusable layout, navigation, cards, panels, forms, tables, alerts, states, typography or repeated CSS blocks are forbidden unless the handoff records a concrete one-off reason. CSS must be managed by owner/role and must not keep inflating `apps/web/src/app/globals.css` with repeated route-specific patterns.
