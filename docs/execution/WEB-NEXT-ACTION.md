...
Objective:
Continue the sequential Real Browser UI/UX Layout pass for the public detailed world gameplay-loop guide after v1.232 `/game/loop` closure. Keep the work page-scoped, Base First and browser-evidence driven.

Current FE scope: select `/guides/world-gameplay-loop-guide` as the next single active page after v1.232 `/game/loop` closure. Complete `/guides/world-gameplay-loop-guide` fully before any other page. Use its registered public route target as the comparison guardrail and prioritize real detailed-guide UI/UX Layout: readable article sections, source-backed step navigation, useful orientation and honest gameplay boundaries, spacing, typography, responsive density, keyboard/focus and actual interaction. No fake live gameplay, damage, HP, inventory, rewards, combat controls, player persistence or server simulation.
...
4. Do not move past `/guides/world-gameplay-loop-guide` until closure evidence exists:
...
6. If execution drifts, return to `/guides/world-gameplay-loop-guide`, inspect rendered layout, check Base First owners, modify the shared base where reusable, and continue until `/guides/world-gameplay-loop-guide` is closed.
