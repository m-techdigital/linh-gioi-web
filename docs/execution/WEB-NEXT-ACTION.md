...
```text
WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT-v1.142
```
...
Current FE scope: select `/release` as the next single active page after v1.141 `/download/trust` closure. Complete `/release` fully before any other page: confirm the registered Public Release target, refresh it if it no longer matches the game scenario or Vietnamese UI direction, implement only `/release` UI/UX Layout and required shared Base UI/UX Layout, run source/runtime/browser/e2e evidence, perform screenshot/design-target comparison, update docs/handoff/ledger, commit and push.
