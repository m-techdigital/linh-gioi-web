...
Next task:
WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT-v1.244

Objective:
Continue the sequential Real Browser UI/UX Layout pass for the public accessibility/readability guide after v1.243 `/guides/player-safety-support-guide` closure. Keep the work page-scoped, Base First and browser-evidence driven.

Current FE scope: select `/guides/accessibility-readability-guide` as the next single active page after v1.243 `/guides/player-safety-support-guide` closure. Complete `/guides/accessibility-readability-guide` fully before any other page. Use its registered public route target as the comparison guardrail and prioritize real accessibility/readability guide UI/UX Layout: clear first steps, full source-authored descriptions, useful orientation and existing reading routes, honest gameplay boundaries, spacing, typography, responsive density, keyboard/focus and actual interaction. No invented guide content, live wiki, account personalization, search backend, game controls or readiness claims.
...
4. Do not move past `/guides/accessibility-readability-guide` until closure evidence exists:
...
6. If execution drifts, return to `/guides/accessibility-readability-guide`, inspect rendered layout, check Base First owners, modify the shared base where reusable, and continue until `/guides/accessibility-readability-guide` is closed.
