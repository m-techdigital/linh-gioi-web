...
Objective:
Continue the sequential Real Browser UI/UX Layout pass for the next single public download trust page after v1.218 `/download` closure. The work must stay page-scoped, Base First and browser-evidence driven.

Current FE scope: select `/download/trust` as the next single active page after v1.218 `/download` closure. Complete `/download/trust` fully before any other page: confirm the registered public route target, apply only the minimal target correction needed if it blocks comparison, then prioritize the real Download Trust UI/UX Layout in browser. Required work is first-fold trust-gate structure, visual hierarchy, spacing, typography scale, evidence density, mobile behavior, keyboard/focus/accessibility, screenshot/design-target comparison, docs/handoff/ledger, commit and push.
...
4. Do not move past `/download/trust` until closure evidence exists:
