...
Objective:
Continue the sequential Real Browser UI/UX Layout pass for the next single public story page after v1.213 `/game` closure. The work must stay page-scoped, Base First and browser-evidence driven.

Current FE scope: select `/story` as the next single active page after v1.213 `/game` closure. Complete `/story` fully before any other page: confirm the registered public route target, apply only the minimal target correction needed if it blocks comparison, then prioritize the real Story UI/UX Layout in browser. Required work is first-fold structure, visual hierarchy, spacing, typography scale, card density, mobile behavior, keyboard/focus/accessibility, screenshot/design-target comparison, docs/handoff/ledger, commit and push.
...
4. Do not move past `/story` until closure evidence exists:
...
6. If execution drifts, stop and reset to this page:
   ...
   - continue until `/story` is closed.
