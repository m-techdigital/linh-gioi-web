...
```text
WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT-v1.157
```
...
Current FE scope: select `/guides/beginner` as the next single active page after v1.156 `/guides` closure. Complete `/guides/beginner` fully before any other page: confirm the registered Public Core target or existing beginner guide target, apply only the minimal target correction needed if it blocks comparison, then prioritize the real beginner guide UI/UX Layout in browser. Required work is first-fold structure, visual hierarchy, spacing, typography scale, card/section density, mobile behavior, keyboard/focus/accessibility, screenshot/design-target comparison, docs/handoff/ledger, commit and push.
