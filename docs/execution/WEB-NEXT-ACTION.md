...
2. Base UI/UX Layout First is mandatory.
   Before adding page-local layout, component or CSS, inspect sibling release/service/support/community/performance/accessibility/roadmap pages, `packages/ui` and `packages/design-tokens`. Reusable hero, proof board, card grid, CTA, form, table, status badge, route map, typography rhythm, responsive density, focus state or CSS block must be extracted or extended in shared owners first.
...
6. If execution drifts, stop and reset to this page:
   ...
   - continue until `/game/loop` is closed.
