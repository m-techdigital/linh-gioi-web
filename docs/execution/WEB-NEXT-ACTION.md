...
Objective:
Continue the sequential Real Browser UI/UX Layout pass for the next single public game overview page after v1.212 `/game/loop` closure. The work must stay page-scoped, Base First and browser-evidence driven.

Current FE scope: select `/game` as the next single active page after v1.212 `/game/loop` closure. Complete `/game` fully before any other page: confirm the registered public route target, apply only the minimal target correction needed if it blocks comparison, then prioritize the real Game overview UI/UX Layout in browser. Required work is first-fold structure, visual hierarchy, spacing, typography scale, card density, mobile behavior, keyboard/focus/accessibility, screenshot/design-target comparison, docs/handoff/ledger, commit and push.
...
2. Base UI/UX Layout First is mandatory.
   Before adding page-local layout, component or CSS, inspect sibling game-loop/roadmap/release/service/support/community/performance/accessibility pages, `packages/ui` and `packages/design-tokens`. Reusable hero, proof board, card grid, CTA, form, table, status badge, route map, typography rhythm, responsive density, focus state or CSS block must be extracted or extended in shared owners first.
...
4. Do not move past `/game` until closure evidence exists:
