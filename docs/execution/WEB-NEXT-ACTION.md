...
Objective:
Continue the sequential Real Browser UI/UX Layout pass for the next single public release page after v1.219 `/download/trust` closure. The work must stay page-scoped, Base First and browser-evidence driven.

Current FE scope: select `/release` as the next single active page after v1.219 `/download/trust` closure. Complete `/release` fully before any other page: confirm the registered public route target, apply only the minimal target correction needed if it blocks comparison, then prioritize the real Release UI/UX Layout in browser. Required work is first-fold release structure, visual hierarchy, spacing, typography scale, readiness/release density, mobile behavior, keyboard/focus/accessibility, screenshot/design-target comparison, docs/handoff/ledger, commit and push.
...
4. Do not move past `/release` until closure evidence exists:
...
6. If execution drifts, stop and reset to this page:
   ...
   - continue until `/release` is closed.
