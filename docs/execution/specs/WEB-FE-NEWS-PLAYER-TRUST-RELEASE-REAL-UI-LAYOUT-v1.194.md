# WEB-FE-NEWS-PLAYER-TRUST-RELEASE-REAL-UI-LAYOUT-v1.194
...
Goal: render a compact Vietnamese News Detail article page that explains player trust and staged release narrative while preserving shared public shell coherence: same header, menu, footer, article rhythm, related-news flow and proof-before-claim wording.
...
Non-claims: NO_ACCEPTED_BACKEND_CONTRACT; no production auth; no DB persistence; no CMS; no live feed; no backend production data; no copied game backend; no copied AXIRO code; no independent business backend; no public download; no beta entitlement; no account flow; no ticket support; no release backend; no production SLA.
