# WEB-FE-NEWS-STATUS-DOWNLOAD-TRUST-REAL-UI-LAYOUT-v1.182
...
Scope: complete only `/news/status-download-trust-polish-started` as the active page from WEB-NEXT-ACTION.

Goal: render a compact Vietnamese News Detail article page that explains status/download trust while preserving shared public shell coherence: same header, menu, footer, article rhythm, related-news flow and backend-boundary wording.
...
Non-claims: NO_ACCEPTED_BACKEND_CONTRACT; no production auth; no DB persistence; no CMS; no live feed; no backend production data; no copied game backend; no independent business backend; no public download artifact or entitlement backend.
