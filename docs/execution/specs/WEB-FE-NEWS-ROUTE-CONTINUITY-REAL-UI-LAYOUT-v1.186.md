# WEB-FE-NEWS-ROUTE-CONTINUITY-REAL-UI-LAYOUT-v1.186
...
Goal: render a compact Vietnamese News Detail article page that explains route continuity and conversion-safe next steps while preserving shared public shell coherence: same header, menu, footer, article rhythm, related-news flow and backend-boundary wording.
...
Non-claims: NO_ACCEPTED_BACKEND_CONTRACT; no production auth; no DB persistence; no CMS; no live feed; no backend production data; no copied game backend; no independent business backend; no fake funnel; no public artifact; no account/support backend; no entitlement/payment/launcher/secure inbox/live moderation claim.
