# WEB-FE-NEWS-PERFORMANCE-COPY-BUDGET-REAL-UI-LAYOUT-v1.192
...
Scope: complete only `/news/performance-copy-asset-budget-polish-started` as the active page from WEB-NEXT-ACTION.

Goal: render a compact Vietnamese News Detail article page that explains performance/copy budget expectations while preserving shared public shell coherence: same header, menu, footer, article rhythm, related-news flow and performance-boundary wording.
...
Non-claims: NO_ACCEPTED_BACKEND_CONTRACT; no production auth; no DB persistence; no CMS; no live feed; no backend production data; no copied game backend; no independent business backend; no production Web Vitals score; no operational monitoring; no CDN release; no bundle-analysis certification; no CDN image integration; no RUM production measurement; no CI production budget; no download entitlement.
