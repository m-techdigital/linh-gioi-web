...
A full browser audit found 27 concrete public routes and16 page-specific visual targets. Do not infer visual completion from historical validators/tests. Process one page at a time using the existing target, real browser screenshots and shared Base First owners. `/story` v1.267, `/classes` v1.268, `/journey` v1.269, `/start` v1.270, `/download` v1.271 and `/download/trust` v1.272 are closed by target-driven browser evidence; current order: `/release`, then the remaining release/service and support/community families. The old `/news` queue is historical, not active authority.
