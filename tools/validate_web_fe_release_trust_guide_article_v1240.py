...
def main() -> int:
    ...
    route = require('apps/web/src/app/guides/[slug]/page.tsx', (
        ...
        'if (entry.slug === "release-trust-and-checksum-guide")', '<PublicReleaseTrustGuide entry={entry}/>'))
    if route.find('notFound();') > route.find('<PublicReleaseTrustGuide entry={entry}/>'): ERRORS.append('category guard must precede specialization')
