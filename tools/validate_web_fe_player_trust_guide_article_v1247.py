...
def main() -> int:
    ...
    route = require('apps/web/src/app/guides/[slug]/page.tsx', (
        ...
        'if (entry.slug === "player-trust-release-guide")', '<PublicPlayerTrustGuide entry={entry}/>'))
    if route.find('notFound();') > route.find('<PublicPlayerTrustGuide entry={entry}/>'): ERRORS.append('category guard must precede specialization')
