#!/usr/bin/env python3

from __future__ import annotations
from pathlib import Path
import re
import sys

ROOT = Path(__file__).resolve().parents[1]
ERRORS: list[str] = []


def fail(message: str) -> None:
    ERRORS.append(message)


def read(rel: str) -> str:
    path = ROOT / rel
    if not path.is_file():
        fail(f"missing readable file: {rel}")
        return ""
    return path.read_text(encoding="utf-8")


def require_file(rel: str) -> None:
    if not (ROOT / rel).is_file():
        fail(f"missing file: {rel}")


def require_text(rel: str, needle: str) -> None:
    if needle not in read(rel):
        fail(f"{rel} missing required text: {needle}")


def check_no_forbidden_roots() -> None:
    for forbidden in ["client", "server", "protocol", "gamedata"]:
        if (ROOT / forbidden).exists():
            fail(f"forbidden game root present: {forbidden}")


def check_no_generated_artifacts() -> None:
    forbidden_parts = {"node_modules", ".next", ".turbo", "dist", "build", "coverage", "__pycache__", ".git"}
    for path in ROOT.rglob("*"):
        if "__pycache__" in path.parts:
            continue
        if forbidden_parts.intersection(path.parts):
            fail(f"forbidden generated/cache artifact present: {path.relative_to(ROOT)}")


def main() -> int:
    for rel in [
        "apps/web/src/components/PublicVisualSections.tsx",
        "docs/execution/WEB-PUBLIC-VISUAL-RESPONSIVE-POLISH-v1.7.md",
        "docs/execution/checklists/WEB-PUBLIC-VISUAL-RESPONSIVE-POLISH-CHECKLIST-v1.7.md",
        "tools/validate_web_public_visual_responsive_polish.py",
    ]:
        require_file(rel)

    for text in [
        "visualPolishItems",
        "responsiveBreakpoints",
        "publicPageFocus",
        "communityPrinciples",
    ]:
        require_text("packages/content/src/fixtures.ts", text)
        require_text("packages/content/src/index.ts", text)

    for text in [
        "SpiritStagePreview",
        "ProductFirstNotice",
        "ResponsiveProofStrip",
        "PublicPageFocusGrid",
    ]:
        require_text("apps/web/src/app/page.tsx", text)
    page_text = read("apps/web/src/app/page.tsx")
    if "WEB v1.7 visual responsive polish" not in page_text and "WEB v1.8 game info depth" not in page_text:
        fail("homepage missing v1.7/v1.8 public visual status wording")

    for text in ["lgo-stage-preview", "lgo-device-strip", "lgo-product-first-band", "@media (max-width: 620px)"]:
        require_text("apps/web/src/app/globals.css", text)

    for rel in [
        "apps/web/src/app/download/page.tsx",
        "apps/web/src/app/support/page.tsx",
        "apps/web/src/app/community/page.tsx",
    ]:
        text = read(rel)
        if "WEB v1.7" not in text and "WEB v1.8" not in text:
            fail(f"{rel} missing WEB v1.7/v1.8 status wording")
    require_text("apps/web/src/app/roadmap/page.tsx", "WEB v1.7")

    for text in ["No production auth", "No DB persistence", "No CMS", "No independent backend", "No production deployment"]:
        require_text("docs/execution/WEB-PUBLIC-VISUAL-RESPONSIVE-POLISH-v1.7.md", text)
        require_text("docs/execution/checklists/WEB-PUBLIC-VISUAL-RESPONSIVE-POLISH-CHECKLIST-v1.7.md", text)

    fixtures = read("packages/content/src/fixtures.ts")
    content_entries_match = re.search(r"export const contentEntries: ContentEntry\[\] = \[([\s\S]*?)\n\];\n\nexport const downloadBuilds", fixtures)
    content_entries_text = content_entries_match.group(1) if content_entries_match else fixtures
    slugs = re.findall(r'slug: "([^"]+)"', content_entries_text)
    if len(slugs) != len(set(slugs)):
        fail("duplicate content entry slug detected")
    if "visual-responsive-polish-started" not in slugs:
        fail("missing v1.7 visual responsive content fixture")

    css = read("apps/web/src/app/globals.css")
    if css.count("@media") < 3:
        fail("expected multiple responsive media query blocks")

    check_no_forbidden_roots()
    check_no_generated_artifacts()

    if ERRORS:
        print("WEB PUBLIC VISUAL RESPONSIVE POLISH VALIDATION FAIL")
        for error in ERRORS:
            print(f"- {error}")
        return 1
    print("WEB PUBLIC VISUAL RESPONSIVE POLISH VALIDATION PASS")
    return 0


if __name__ == "__main__":
    sys.exit(main())
