#!/usr/bin/env python3
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
ERRORS: list[str] = []

def fail(message: str) -> None:
    ERRORS.append(message)

def read(rel: str) -> str:
    path = ROOT / rel
    if not path.is_file():
        fail(f"missing file: {rel}")
        return ""
    return path.read_text(encoding="utf-8")

def require_file(rel: str) -> None:
    if not (ROOT / rel).is_file():
        fail(f"missing file: {rel}")

def require_text(rel: str, markers: list[str]) -> str:
    text = read(rel)
    for marker in markers:
        if marker not in text:
            fail(f"{rel}: missing {marker}")
    return text

def check_safety_board() -> None:
    require_file("apps/web/public/design-reference/support-safety-detailed-design-target-v1133.png")
    require_text("apps/web/src/app/support/safety/page.tsx", [
        "lgo-safety-support-design-board",
        "lgo-service-proof-board",
        "/design-reference/support-safety-detailed-design-target-v1133.png",
        "Bảng thiết kế báo lỗi an toàn",
        "loading=\"eager\"",
        "Board tham chiếu",
        "chưa có ticket thật",
    ])
    require_text("packages/ui/src/service-layout.css", [
        "Shared support/safety safe-reporting layout",
        "lgo-safety-support-design-board",
        "lgo-support-safety-checklist",
        "lgo-player-safety-board",
        "lgo-support-issue-path-board",
        "grid-template-columns: repeat(2, minmax(0, 1fr))",
    ])

def check_tests_and_docs() -> None:
    for rel in [
        "tests/e2e/fe-public-safety-support-design-board-v172.spec.ts",
        "docs/execution/specs/WEB-FE-PUBLIC-SAFETY-SUPPORT-DESIGN-BOARD-v1.72.md",
        "LGO-WEB-FE-PUBLIC-SAFETY-SUPPORT-DESIGN-BOARD-REPORT-v1.72.md",
        "HANDOFF-LGO-WEB-FE-PUBLIC-SAFETY-SUPPORT-DESIGN-BOARD-v1.72.md",
    ]:
        require_file(rel)
    require_text("tests/e2e/fe-public-safety-support-design-board-v172.spec.ts", [
        "/support/safety",
        "Player safety support HUD board",
        "/game-art/design-boards/player-safety-support-hud.svg",
        "loading",
        "naturalWidth",
        "safety support horizontal overflow",
    ])
    for rel in [
        "docs/execution/specs/WEB-FE-PUBLIC-SAFETY-SUPPORT-DESIGN-BOARD-v1.72.md",
        "LGO-WEB-FE-PUBLIC-SAFETY-SUPPORT-DESIGN-BOARD-REPORT-v1.72.md",
        "HANDOFF-LGO-WEB-FE-PUBLIC-SAFETY-SUPPORT-DESIGN-BOARD-v1.72.md",
    ]:
        require_text(rel, [
            "WEB-FE-PUBLIC-SAFETY-SUPPORT-DESIGN-BOARD-v1.72",
            "WEB_CLOSED",
            "Player safety support HUD board",
            "browser/e2e",
            "No production auth",
            "No DB persistence",
            "No real Portal integration",
            "No real Ops/Admin mutation",
            "NO_ACCEPTED_BACKEND_CONTRACT",
        ])
    require_text("docs/execution/WEB-PROJECT-STATE.md", [
        "Current phase: WEB-FE-PUBLIC-SAFETY-SUPPORT-DESIGN-BOARD-v1.72 WEB_CLOSED",
        "WEB-FE-PUBLIC-SAFETY-SUPPORT-DESIGN-BOARD-v1.72",
    ])
    require_text("docs/execution/WEB-TASK-LEDGER.md", [
        "| WEB-FE-PUBLIC-SAFETY-SUPPORT-DESIGN-BOARD-v1.72 | WEB-FE | WEB_CLOSED |",
    ])
    require_text("docs/execution/WEB-NEXT-ACTION.md", [
        "WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT",
        "browser/e2e",
    ])

def main() -> int:
    check_safety_board()
    check_tests_and_docs()
    if ERRORS:
        print("WEB FE PUBLIC SAFETY SUPPORT DESIGN BOARD v1.72 VALIDATION FAIL")
        for error in ERRORS:
            print(f"- {error}")
        return 1
    print("WEB FE PUBLIC SAFETY SUPPORT DESIGN BOARD v1.72 VALIDATION PASS")
    return 0

if __name__ == "__main__":
    raise SystemExit(main())
