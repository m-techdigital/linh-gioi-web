...
def main()->int:
 ...
 view=require('apps/web/src/components/PublicEventsExperience.tsx',('title="Sự kiện Linh Giới"','AnnouncementBoard','entries.map(entry =>','id: entry.slug','title: entry.title','summary: entry.summary','body: entry.body','iso: entry.publishedAt','publicationDate.format(new Date(entry.publishedAt))','timeZone: "UTC"','Chưa có lịch live, đăng ký tham gia hoặc phần thưởng sự kiện.','Ngày đăng nội dung không phải ngày tổ chức sự kiện.','Không phải hình sự kiện đang diễn ra','NO_ACCEPTED_BACKEND_CONTRACT','<ArticleFragmentRestoration targetIds={["events-announcements"]}/>','id="events-announcements"','tabIndex={-1}','/status','/roadmap','/community','/support/help','emptyTitle="Chưa có thông báo sự kiện công khai"'))
