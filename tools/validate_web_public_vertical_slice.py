...
def main() -> int:
    ...
    for rel in [
        ...
        require_file(rel)
    for route in ["/game", "/news", "/download", "/support"]:
        require_text("apps/web/src/components/PublicNavigation.tsx", route)
    require_text("apps/web/src/app/layout.tsx", "openGraph")
    require_text("apps/web/src/components/PublicSiteShell.tsx", "SiteFooter")
    require_text("apps/web/src/app/download/page.tsx", "No public production download is available")
    require_text("packages/content/src/fixtures.ts", "PROVISIONAL_WEB_FIXTURE")
    require_text("packages/content/src/fixtures.ts", "NOT_CANONICAL_BACKEND_CONTRACT")
    return finish("WEB PUBLIC VERTICAL SLICE")
