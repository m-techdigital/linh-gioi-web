...
def check_target() -> None:
    ...
    for rel in targets:
        ...
        if width < 1600 or height < 900: fail(f"{rel}: expected high-fidelity status target dimensions, got {width}x{height}")
...
def check_tests_docs() -> None:
    ...
    for line in registry.splitlines():
        if line.startswith("| Public Service |") and "`/status`" in line:
            fail("docs/design/DESIGN-TARGET-REGISTRY.md: /status should not remain under broad Public Service applies-to list")
    for rel in ["docs/execution/specs/WEB-FE-STATUS-DESIGN-TARGET-DENSITY-v1.130.md", "LGO-WEB-FE-STATUS-DESIGN-TARGET-DENSITY-REPORT-v1.130.md", "HANDOFF-LGO-WEB-FE-STATUS-DESIGN-TARGET-DENSITY-v1.130.md"]:
        require_text(rel, ["WEB-FE-STATUS-DESIGN-TARGET-DENSITY-v1.130", "WEB_CLOSED", "Design Target First", "Base UI/UX Layout", "Public Status", "browser/e2e", "fold density", "built-in image_gen", "No production auth", "No DB persistence", "No real Portal integration", "No real Ops/Admin mutation", "NO_ACCEPTED_BACKEND_CONTRACT"])
    require_text("docs/execution/WEB-PROJECT-STATE.md", ["Current phase: WEB-FE-STATUS-DESIGN-TARGET-DENSITY-v1.130 WEB_CLOSED", "Next task: WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT-v1.131"])
    require_text("docs/execution/WEB-NEXT-ACTION.md", ["WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT-v1.131", "Design Target First", "Base UI/UX Layout", "browser/e2e"])
    require_text("docs/execution/WEB-TASK-LEDGER.md", ["| WEB-FE-STATUS-DESIGN-TARGET-DENSITY-v1.130 | WEB-FE | WEB_CLOSED |"])
def main() -> int:
    check_target(); check_tests_docs()
    if ERRORS:
        print("WEB FE STATUS DESIGN TARGET DENSITY v1.130 VALIDATION FAIL")
        for error in ERRORS: print(f"- {error}")
        return 1
    print("WEB FE STATUS DESIGN TARGET DENSITY v1.130 VALIDATION PASS")
    return 0
