#!/usr/bin/env python3
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
ERRORS: list[str] = []

def fail(message: str) -> None:
    ERRORS.append(message)

def read(rel: str) -> str:
    p = ROOT / rel
    if not p.is_file():
        fail(f"missing file: {rel}")
        return ""
    return p.read_text(encoding="utf-8")

def require_file(rel: str) -> None:
    if not (ROOT / rel).is_file():
        fail(f"missing file: {rel}")

def require_text(rel: str, markers: list[str]) -> None:
    text = read(rel)
    for marker in markers:
        if marker not in text:
            fail(f"{rel}: missing {marker}")

def check_tests_docs() -> None:
    for rel in [
        "tests/e2e/fe-homepage-target-fold-density-v1119.spec.ts",
        "docs/execution/specs/WEB-FE-HOMEPAGE-TARGET-FOLD-DENSITY-v1.119.md",
        "LGO-WEB-FE-HOMEPAGE-TARGET-FOLD-DENSITY-REPORT-v1.119.md",
        "HANDOFF-LGO-WEB-FE-HOMEPAGE-TARGET-FOLD-DENSITY-v1.119.md",
    ]:
        require_file(rel)
    require_text("tests/e2e/fe-homepage-target-fold-density-v1119.spec.ts", [
        "homepage target fold density",
        "Public Homepage",
        "desktop first card enters first fold",
        "desktop first card visible in first fold",
        "desktop hero height leaves room for target cards",
        "homepage h1 font-size follows target scale",
        "homepage identity signal chips",
    ])
    require_text("apps/web/src/app/globals.css", [
        "WEB v1.119 homepage target fold density",
        ".lgo-cinematic-hero",
        ".lgo-player-facing-stack",
    ])
    require_text("docs/design/DESIGN-TARGET-REGISTRY.md", ["Public Homepage", "homepage-detailed-design-target-v1118.png"])
    for rel in [
        "docs/execution/specs/WEB-FE-HOMEPAGE-TARGET-FOLD-DENSITY-v1.119.md",
        "LGO-WEB-FE-HOMEPAGE-TARGET-FOLD-DENSITY-REPORT-v1.119.md",
        "HANDOFF-LGO-WEB-FE-HOMEPAGE-TARGET-FOLD-DENSITY-v1.119.md",
    ]:
        require_text(rel, [
            "WEB-FE-HOMEPAGE-TARGET-FOLD-DENSITY-v1.119",
            "WEB_CLOSED",
            "Design Target First",
            "Base UI/UX Layout",
            "Public Homepage",
            "browser/e2e",
            "fold density",
            "No production auth",
            "No DB persistence",
            "No real Portal integration",
            "No real Ops/Admin mutation",
            "NO_ACCEPTED_BACKEND_CONTRACT",
        ])
    require_text("docs/execution/WEB-PROJECT-STATE.md", [
        "Current phase: WEB-FE-HOMEPAGE-TARGET-FOLD-DENSITY-v1.119 WEB_CLOSED",
        "Design Target First",
    ])
    require_text("docs/execution/WEB-NEXT-ACTION.md", ["WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT", "Design Target First", "Base UI/UX Layout", "browser/e2e"])
    require_text("docs/execution/WEB-TASK-LEDGER.md", ["| WEB-FE-HOMEPAGE-TARGET-FOLD-DENSITY-v1.119 | WEB-FE | WEB_CLOSED |"])

def main() -> int:
    check_tests_docs()
    if ERRORS:
        print("WEB FE HOMEPAGE TARGET FOLD DENSITY v1.119 VALIDATION FAIL")
        for error in ERRORS:
            print(f"- {error}")
        return 1
    print("WEB FE HOMEPAGE TARGET FOLD DENSITY v1.119 VALIDATION PASS")
    return 0

if __name__ == "__main__":
    raise SystemExit(main())
