...
def main() -> int:
    ...
    route = require('apps/web/src/app/guides/[slug]/page.tsx', (
        ...
        'if (entry.slug === "route-continuity-conversion-guide")', '<PublicRouteContinuityGuide entry={entry}/>'))
    if route.find('notFound();') > route.find('<PublicRouteContinuityGuide entry={entry}/>'): ERRORS.append('category guard must precede specialization')
