...
def check_tests_docs() -> None:
    ...
    require_text("docs/execution/WEB-PROJECT-STATE.md", ["WEB-FE-SUPPORT-SAFETY-REAL-UI-LAYOUT-v1.148 WEB_CLOSED", "Next task: WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT-v1.173", "Select `/community/onboarding` as the next single active page"])
    require_text("docs/execution/WEB-NEXT-ACTION.md", ["WEB-FE-ACCESSIBILITY-INTERACTION-AUDIT-v1.173", "Current FE scope: select `/guides/faq-search-helpfulness-guide`", "Real Browser UI/UX Layout First is Priority #1", "Base UI/UX Layout", "CSS must be managed by owner/role"])
