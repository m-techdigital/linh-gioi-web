...
def main()->int:
 ERRORS.clear()
 route=require('apps/web/src/app/news/[slug]/page.tsx',('localContentRepository.list("news")','if (!entry || entry.category !== "news") notFound();','if (entry.slug === "web-program-control-tower")','<PublicControlTowerArticle entry={entry} related={related}/>','.filter((candidate) => candidate.slug !== entry.slug)','.slice(0, 3)','<ArticleDetailDepth slug={entry.slug} />'))
 if route.find('notFound();')>route.find('<PublicControlTowerArticle'):ERRORS.append('category guard must precede exact article specialization')
 view=require('apps/web/src/components/PublicControlTowerArticle.tsx',('contentDetailSections.filter(section => section.slug === entry.slug)','title={entry.title}','lead={entry.summary}','{entry.body}','dateTime={entry.publishedAt}','timeZone: "UTC"','publicationDate.format(new Date(entry.publishedAt))','Ngày đăng nội dung','không phải thông cáo phát hành game','GuideArticle','Mục lục bài viết Control tower','contentsId={contentsId}','title: section.heading','instruction={section.body}','outcome={section.playerImpact}','boundary={section.nonClaim}','outcomeLabel="Ý nghĩa cho người chơi"','boundaryLabel="Phạm vi bài viết"','chapterLinks[index]','lgo-editorial-article','GuidanceTopicGrid variant="articles"','title: item.title','description: item.summary','href: `/news/${item.slug}`','related.length > 0','/status','/release','/guides','/news','/roadmap','NO_ACCEPTED_BACKEND_CONTRACT'))
 ...
 for owner,text in [('view',view),('chapter',chapter)]:
