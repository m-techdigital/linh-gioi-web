...
test('immersive shell renders compact marketing footer instead of the legacy generic footer',()=>{const html=renderToStaticMarkup(createElement(PublicSiteShell,{variant:'immersive'},createElement('div',null,'content')));expect(html).toContain('lgo-marketing-footer');expect(html).toContain('/game-art/marketing/wordmark-brush.png');expect(html).toContain('Bản công khai chưa mở');expect(html).not.toContain('lgo-brand-footer-inner');expect(html).not.toContain('Máy chủ hoạt động ổn định');});
test('default public shell uses the same canonical marketing footer as homepage',()=>{const html=renderToStaticMarkup(createElement(PublicSiteShell,null,createElement('div',null,'content')));expect(html).toContain('lgo-marketing-footer');expect(html).toContain('/game-art/marketing/wordmark-brush.png');expect(html).toContain('Bản công khai chưa mở');expect(html).not.toContain('lgo-brand-footer-inner');});
