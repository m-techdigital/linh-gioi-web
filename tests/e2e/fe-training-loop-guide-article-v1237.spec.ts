...
const route = '/guides/beginner-training-loop-guide';
const entry = contentEntries.find(item => item.slug === 'beginner-training-loop-guide')!;
const steps = guideDetailSteps.filter(item => item.slug === entry.slug);

test.describe('training loop is a reading guide, not a running game v1.237', () => {
  ...
  test('four full article chapters and responsive contents replace compact proof boards', async ({ page, isMobile }) => {
    const guide = page.locator('.lgo-training-loop-guide');
    ...
    await page.screenshot({ path: test.info().outputPath('training-loop-article.png'), fullPage: true });
  });
  test('original summary body and all four instruction result boundary records are exact', async ({ page }) => {
    const guide = page.locator('.lgo-training-loop-guide'); await expect(guide).toBeVisible();
  ...
  test('native contents next previous and browser history follow the real chapter IDs', async ({ page }) => {
    const contents = page.getByRole('navigation', { name: 'Mục lục luyện tập' });
    const links = contents.locator('a'); await expect(links).toHaveCount(4);
    await links.nth(1).focus(); await page.keyboard.press('Enter'); await expect(page.locator('#training-loop-step-02')).toBeFocused();
    await page.locator('#training-loop-step-02').getByRole('link', { name: /Phần tiếp/ }).click();
    await expect(page).toHaveURL(origin + route + '#training-loop-step-03'); await expect(page.locator('#training-loop-step-03')).toBeFocused();
    await page.goBack(); await expect(page).toHaveURL(origin + route + '#training-loop-step-02');
    await page.goForward(); await expect(page).toHaveURL(origin + route + '#training-loop-step-03');
    await page.locator('#training-loop-step-03').getByRole('link', { name: 'Về mục lục', exact: true }).click();
    await expect(page.locator('#training-loop-contents')).toBeFocused();
    const summary = page.locator('#training-loop-contents summary'); await summary.focus(); await page.keyboard.press('Space');
  ...
  test('cold direct URLs land visibly at the requested chapter and malformed fragments are harmless', async ({ page }) => {
    await page.goto('about:blank'); await page.goto(origin + route + '#training-loop-step-03', { waitUntil: 'networkidle' });
    const section = page.locator('#training-loop-step-03'); await expect(section).toBeFocused();
  ...
  test('chapter actions and related destinations navigate to existing guidance, not commands', async ({ page }) => {
    const actions = page.locator('.lgo-training-loop-action'); await expect(actions).toHaveCount(4);
    const routes = ['/game', '/guides/gate-entry-guide', '/game/loop', '/download/trust'];
    ...
    await expect(actions.nth(1)).toHaveAttribute('href','/guides/gate-entry-guide');
  ...
  test('source paper typography and actions remain legible at 320px with visible focus', async ({ page }) => {
    ...
    for (const link of await guide.locator('.lgo-article-section-navigation a,.lgo-training-loop-action').all()) expect((await link.boundingBox())!.height).toBeGreaterThanOrEqual(44);
    const action = guide.locator('.lgo-training-loop-action').first(); await action.focus(); await expect(action).toBeFocused();
  ...
  test('reading has no submission saved progress access award or gameplay surrogate', async ({ page }) => {
    const guide = page.locator('.lgo-training-loop-guide'); await expect(guide).toBeVisible();
    await expect(guide).toContainText('Đang đọc hướng dẫn, không phải đang luyện trong game'); await expect(guide).toContainText('NO_ACCEPTED_BACKEND_CONTRACT');
    ...
    await page.getByRole('navigation', { name: 'Mục lục luyện tập' }).locator('a').first().click();
    await page.locator('#training-loop-step-01').getByRole('link', { name: /Phần tiếp/ }).click();
