...
import {contentEntries,contentDetailSections,supportFaqs,playerSafetyPrinciples} from '../../packages/content/src/fixtures';
const origin=process.env.LGO_WEB_URL??'http://127.0.0.1:3000';
const slug='player-safety-support-faq-polish-started',route=`/news/${slug}`;
...
test.describe('source-complete safety and support article v1.264',()=>{
 ...
 test('reading layout and responsive contents replace compact proof cards',async({page,isMobile})=>{
  ...
  await page.screenshot({path:test.info().outputPath('news-safety-support-article.png'),fullPage:true});
 });
 ...
 test('contents previous next and Back Forward move between the real sections',async({page})=>{
  const links=page.getByRole('navigation',{name:'Mục lục bài viết an toàn và hỗ trợ'}).getByRole('link');await expect(links).toHaveCount(2);
 ...
 test('context links and breadcrumbs navigate to real published routes',async({page})=>{
  ...
  for(const [i,href] of ['/support/safety','/support/help','/guides/player-safety-support-guide','/community','/support','/status','/download/trust','/release/tester-pack'].entries()){await expect(links.nth(i)).toHaveAttribute('href',href);expect((await page.request.get(origin+href)).status()).toBe(200);await links.nth(i).focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+href);await page.goBack();}
 ...
 test('reduced motion uses immediate native section arrival rather than smooth scrolling',async({page})=>{
  ...
  await page.getByRole('navigation',{name:'Mục lục bài viết an toàn và hỗ trợ'}).getByRole('link').last().click();
 ...
 test('resizing the same article retains complete chapters and a single coherent contents navigation',async({page})=>{
  await page.emulateMedia({reducedMotion:'reduce'});
  const nav=page.getByRole('navigation',{name:'Mục lục bài viết an toàn và hỗ trợ'});
 ...
 test('expanded text spacing does not clip authored text or hide reading links',async({page})=>{
  ...
  await root.locator('.lgo-news-article-action').last().focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+'/release/tester-pack');
 });

 test('safety destination checklist changes only local marks and resets without claiming redaction or submission',async({page})=>{
  await page.locator('.lgo-news-article-action[href="/support/safety"]').focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+'/support/safety');
  const list=page.locator('#safety-checklist .lgo-local-checklist'),boxes=list.getByRole('checkbox');await expect(boxes).toHaveCount(5);await expect(list.getByRole('status')).toContainText('0/5');
  ...
  for(let i=0;i<5;i++){const box=boxes.nth(i);await box.focus();await page.keyboard.press('Space');await expect(box).toBeChecked();await expect(box).toBeFocused();await expect(list.getByRole('status')).toContainText(`${i+1}/5`);}
  await expect(page.locator('#safety-checklist')).toContainText('không tự che ảnh/log');await expect(page.locator('#safety-no-intake')).toContainText('Chưa có ticket thật');await expect(page.locator('main form,main input:not([type=checkbox]),main textarea,main a[download]')).toHaveCount(0);
  await page.screenshot({path:test.info().outputPath('safety-checklist-from-news.png'),fullPage:true});
  const reset=list.getByRole('button',{name:'Bỏ các đánh dấu',exact:true});await reset.focus();await page.keyboard.press('Enter');await expect(list.getByRole('status')).toContainText('0/5');for(const box of await boxes.all())await expect(box).not.toBeChecked();expect(requests).toEqual([]);expect(await storage()).toEqual(before);
  await boxes.first().check();await page.reload();await expect(list.getByRole('status')).toContainText('0/5');await page.goBack();await expect(page).toHaveURL(origin+route);
 });
 test('FAQ destination filters real reporting and privacy answers and returns without opening a ticket',async({page})=>{
  const href='/support/help';await page.locator(`.lgo-news-article-action[href="${href}"]`).click();await expect(page).toHaveURL(origin+href);
  const directory=page.locator('.lgo-question-directory'),controls=directory.getByRole('group',{name:'Chọn chủ đề câu hỏi'});await expect(directory.locator('.lgo-question-group:visible')).toHaveCount(6);
  await page.waitForLoadState('networkidle');const requests:string[]=[];page.on('request',r=>{if(r.method()!=='GET'||['fetch','xhr'].includes(r.resourceType()))requests.push(r.url());});const storage=()=>page.evaluate(()=>({local:{...localStorage},session:{...sessionStorage}}));const before=await storage();
  const reporting=controls.getByRole('button',{name:'Báo lỗi an toàn',exact:true});await reporting.focus();await page.keyboard.press('Space');await expect(reporting).toHaveAttribute('aria-pressed','true');await expect(directory.locator('.lgo-question-group:visible')).toHaveCount(1);await expect(directory.getByRole('status')).toContainText('1/6');
  const report=page.locator('#faq-report'),answer=report.locator('details').first();await answer.locator('summary').focus();await page.keyboard.press('Enter');await expect(answer).toHaveAttribute('open','');await expect(answer.locator('.lgo-question-answer p').first()).toHaveText(supportFaqs.find(x=>x.scope==='privacy-safe issue reporting')!.answer);await expect(report.locator(':scope > a')).toHaveAttribute('href','/support/safety');
  const privacy=controls.getByRole('button',{name:'Ranh giới dữ liệu',exact:true});await privacy.focus();await page.keyboard.press('Enter');await expect(privacy).toHaveAttribute('aria-pressed','true');const privacyGroup=page.locator('#faq-privacy');await privacyGroup.locator('summary').first().click();const source=playerSafetyPrinciples.find(x=>x.id==='privacy-first-reports')!;await expect(privacyGroup).toContainText(source.playerCopy);await expect(privacyGroup).toContainText(source.notClaimed);await expect(page.locator('#help-boundary')).toContainText('Không có hệ thống ticket thật');await expect(page.locator('main form,main input,main textarea')).toHaveCount(0);expect(requests).toEqual([]);expect(await storage()).toEqual(before);
  await page.screenshot({path:test.info().outputPath('faq-privacy-from-news.png'),fullPage:true});
  await controls.getByRole('button',{name:'Tất cả',exact:true}).click();await expect(directory.locator('.lgo-question-group:visible')).toHaveCount(6);await page.reload();await expect(directory.locator('.lgo-question-group:visible')).toHaveCount(6);await page.goto(origin+route);await expect(page.getByRole('heading',{level:1,name:entry.title,exact:true})).toBeVisible();
 });
