// v1.181 coverage: /news/news-guide-detail-pages-started must render as compact Vietnamese news/guide detail article, not a raw WEB v1.9 fixture body.
...
test.describe("news guide detail real UI layout v1.181", () => {
  test("/news/news-guide-detail-pages-started renders compact Vietnamese news guide detail article flow", async ({ page, isMobile }) => {
    await page.goto(`${web}/news/news-guide-detail-pages-started`);

    await expect(page.getByRole("heading", { level: 1, name: "News và guide detail được làm sâu hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích news và guide detail" })).toBeVisible();
    ...
    expect(mainText).not.toMatch(/WEB v1\.9 keeps the product focus|article detail sections, guide detail steps|no CMS, no production auth, no DB persistence|live support backend|public game download artifact|WEB v1\.9 article detail|Boundary:|PROVISIONAL_WEB_FIXTURE|Detail section|This public content is/i);
    ...
    await page.screenshot({ path: `/tmp/news-guide-detail-${isMobile ? "mobile" : "desktop"}-v1181.png`, fullPage: true });
  });
});
