...
import {contentEntries,contentDetailSections,roadmapDecisionGates,stagedReleaseMessages} from '../../packages/content/src/fixtures';
const origin=process.env.LGO_WEB_URL??'http://127.0.0.1:3000';
const slug='community-roadmap-onboarding-started',route=`/news/${slug}`;
...
test.describe('source-complete community and roadmap article v1.261',()=>{
 ...
 test('reading layout and responsive contents replace compact proof cards',async({page,isMobile})=>{
  ...
  await page.screenshot({path:test.info().outputPath('news-community-article.png'),fullPage:true});
 });
 ...
 test('contents previous next and Back Forward move between the real sections',async({page})=>{
  const links=page.getByRole('navigation',{name:'Mục lục bài viết cộng đồng và lộ trình'}).getByRole('link');await expect(links).toHaveCount(2);
 ...
 test('reduced motion uses immediate native section arrival rather than smooth scrolling',async({page})=>{
  ...
  await page.getByRole('navigation',{name:'Mục lục bài viết cộng đồng và lộ trình'}).getByRole('link').last().click();
 ...
 test('resizing the same article retains complete chapters and a single coherent contents navigation',async({page})=>{
  await page.emulateMedia({reducedMotion:'reduce'});
  const nav=page.getByRole('navigation',{name:'Mục lục bài viết cộng đồng và lộ trình'});
 ...
 test('expanded text spacing does not clip authored text or hide reading links',async({page})=>{
  ...
  await root.locator('.lgo-news-article-action').last().focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+'/release/readiness');
 });

 test('the onboarding destination keeps its three actual reading steps local and resets after reload',async({page})=>{
  await page.locator('.lgo-news-article-action[href="/community/onboarding"]').click();await expect(page).toHaveURL(origin+'/community/onboarding');
  const journey=page.locator('.lgo-reading-journey');await expect(journey).toBeVisible();await expect(journey.getByRole('status')).toContainText('1/3');await expect(journey.getByRole('button',{name:'Bước trước',exact:true})).toBeDisabled();
  ...
  for(let i=0;i<3;i++){
   const button=journey.getByRole('button',{name:new RegExp(`^Xem bước ${i+1}:`)});await button.focus();await page.keyboard.press('Space');await expect(button).toBeFocused();await expect(button).toHaveAttribute('aria-pressed','true');await expect(journey.locator('.lgo-progress-step[aria-current=step]')).toHaveCount(1);await expect(journey.getByRole('status')).toContainText(`${i+1}/3`);
   await expect(journey.locator('.lgo-reading-journey-panel').getByRole('link')).toHaveAttribute('href',['/status','/roadmap','/community'][i]);
  }
  await expect(journey.getByRole('button',{name:'Bước tiếp',exact:true})).toBeDisabled();await expect(page.locator('#onboarding-boundary')).toContainText('Chưa có danh sách chờ');expect(requests).toEqual([]);expect(await storage()).toEqual(before);
  await page.screenshot({path:test.info().outputPath('onboarding-step-from-news.png')});await page.reload();await expect(journey.getByRole('status')).toContainText('1/3');await page.goBack();await expect(page).toHaveURL(origin+route);
 });
 test('roadmap source gates and stage details open without approval or public visibility changing',async({page})=>{
  await page.locator('.lgo-news-article-action[href="/roadmap"]').click();await expect(page).toHaveURL(origin+'/roadmap');const gates=page.locator('.lgo-planning-gate'),stages=page.locator('.lgo-planning-stage');await expect(gates).toHaveCount(roadmapDecisionGates.length);await expect(stages).toHaveCount(stagedReleaseMessages.length);
  await page.waitForLoadState('networkidle');const requests:string[]=[];page.on('request',r=>{if(r.method()!=='GET'||['fetch','xhr'].includes(r.resourceType()))requests.push(r.url());});
  for(const [i,source]of roadmapDecisionGates.entries()){
   const card=gates.nth(i);await expect(card).toHaveAttribute('data-state',source.status);const summary=card.locator('summary');await summary.focus();await page.keyboard.press('Enter');await expect(card.locator('details')).toHaveAttribute('open','');await expect(card).toContainText(source.decisionOwner);await expect(card).toContainText(source.mustNotClaim);await expect(card).toHaveAttribute('data-state',source.status);
  }
  for(const [i,source]of stagedReleaseMessages.entries()){
   const card=stages.nth(i);await expect(card).toHaveAttribute('data-visibility',source.visibility);await card.locator('summary').focus();await page.keyboard.press('Space');await expect(card.locator('details')).toHaveAttribute('open','');await expect(card).toContainText(source.playerCopy);await expect(card).toContainText(source.nonClaim);await expect(card).toHaveAttribute('data-visibility',source.visibility);
  }
  expect(requests).toEqual([]);await expect(page.locator('main form,main input,main a[download]')).toHaveCount(0);await expect(page.locator('main')).toContainText('Không phải lịch phát hành');await page.screenshot({path:test.info().outputPath('roadmap-stages-from-news.png')});await page.goBack();await expect(page).toHaveURL(origin+route);
 });
