...
test.describe('events are readable announcements, not live operations v1.251',()=>{
  test.beforeEach(async({page})=>{await page.goto(origin+'/events');await expect(page.getByRole('heading',{level:1,name:'Sự kiện Linh Giới',exact:true})).toBeVisible();});
  ...
  test('announcement and boundary links arrive below the header and keep native history',async({page})=>{
    await page.getByRole('link',{name:'Đọc thông báo hiện có',exact:true}).click();
