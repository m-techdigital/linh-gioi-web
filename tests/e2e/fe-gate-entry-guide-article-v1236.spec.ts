...
const route = '/guides/gate-entry-guide';
const entry = contentEntries.find(item => item.slug === 'gate-entry-guide')!;
const steps = guideDetailSteps.filter(item => item.slug === entry.slug);

test.describe('gate entry is an authored guide, not a running game v1.236', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(origin + route);
    await expect(page.getByRole('heading', { level: 1, name: entry.title, exact: true })).toBeVisible();
  });
  test('three full article chapters and responsive contents replace compact proof boards', async ({ page, isMobile }) => {
    const guide = page.locator('.lgo-gate-entry-guide');
    await expect(guide).toBeVisible();
    await expect(guide.locator('.lgo-guide-article-section')).toHaveCount(3);
    ...
    await page.screenshot({ path: test.info().outputPath('gate-entry-article.png'), fullPage: true });
  });
  test('original summary body and all three instruction result boundary records are exact', async ({ page }) => {
    const guide = page.locator('.lgo-gate-entry-guide'); await expect(guide).toBeVisible();
  ...
  test('native contents next previous and browser history follow the real chapter IDs', async ({ page }) => {
    const contents = page.getByRole('navigation', { name: 'Mục lục Cổng Linh' });
    const links = contents.locator('a'); await expect(links).toHaveCount(3);
    await links.nth(1).focus(); await page.keyboard.press('Enter'); await expect(page.locator('#gate-entry-step-02')).toBeFocused();
    await page.locator('#gate-entry-step-02').getByRole('link', { name: /Phần tiếp/ }).click();
    await expect(page).toHaveURL(origin + route + '#gate-entry-step-03'); await expect(page.locator('#gate-entry-step-03')).toBeFocused();
    await page.goBack(); await expect(page).toHaveURL(origin + route + '#gate-entry-step-02');
    await page.goForward(); await expect(page).toHaveURL(origin + route + '#gate-entry-step-03');
    await page.locator('#gate-entry-step-03').getByRole('link', { name: 'Về mục lục', exact: true }).click();
    await expect(page.locator('#gate-entry-contents')).toBeFocused();
    const summary = page.locator('#gate-entry-contents summary'); await summary.focus(); await page.keyboard.press('Space');
  ...
  test('cold direct URLs land visibly at the requested chapter and malformed fragments are harmless', async ({ page }) => {
    await page.goto('about:blank'); await page.goto(origin + route + '#gate-entry-step-02', { waitUntil: 'networkidle' });
    const section = page.locator('#gate-entry-step-02'); await expect(section).toBeFocused();
    ...
    await expect(page.locator('.lgo-guide-article-section')).toHaveCount(3); expect(errors).toEqual([]);
  });
  test('chapter actions and related destinations navigate to existing guidance, not commands', async ({ page }) => {
    const actions = page.locator('.lgo-gate-entry-action'); await expect(actions).toHaveCount(3);
    const routes = ['/game', '/game/loop', '/download/trust'];
    ...
    await actions.last().click(); await expect(page).toHaveURL(origin + '/download/trust');
  });
  test('source paper typography and actions remain legible at 320px with visible focus', async ({ page }) => {
    ...
    for (const link of await guide.locator('.lgo-article-section-navigation a,.lgo-gate-entry-action').all()) expect((await link.boundingBox())!.height).toBeGreaterThanOrEqual(44);
    const action = guide.locator('.lgo-gate-entry-action').first(); await action.focus(); await expect(action).toBeFocused();
  ...
  test('reading has no submission saved progress access award or gameplay surrogate', async ({ page }) => {
    const guide = page.locator('.lgo-gate-entry-guide'); await expect(guide).toBeVisible();
    await expect(guide).toContainText('Đang đọc cẩm nang, không phải đang chơi'); await expect(guide).toContainText('NO_ACCEPTED_BACKEND_CONTRACT');
    await expect(guide.locator('canvas,video,iframe,form,input,textarea,[role=progressbar]')).toHaveCount(0);
    const requests: string[] = []; page.on('request', r => { if (r.method() !== 'GET' || ['xhr', 'fetch'].includes(r.resourceType())) requests.push(r.url()); });
    await page.getByRole('navigation', { name: 'Mục lục Cổng Linh' }).locator('a').first().click();
    await page.locator('#gate-entry-step-01').getByRole('link', { name: /Phần tiếp/ }).click();
