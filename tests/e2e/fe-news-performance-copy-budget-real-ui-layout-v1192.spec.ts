// v1.192 coverage: /news/performance-copy-asset-budget-polish-started must render compact Vietnamese performance copy budget article with detail cards, not a body-only performance fixture.
...
test.describe("news performance copy budget real UI layout v1.192", () => {
  test("/news/performance-copy-asset-budget-polish-started renders compact Vietnamese performance copy budget article flow", async ({ page, isMobile }) => {
    await page.goto(`${web}/news/performance-copy-asset-budget-polish-started`);

    await expect(page.getByRole("heading", { level: 1, name: "Web nhẹ và rõ hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích hiệu năng và ngân sách nội dung" })).toBeVisible();
    ...
    expect(mainText).not.toMatch(/Bài viết giải thích nội dung public|This update keeps product focus|Web Vitals PASS|production monitoring|CDN deployment|bundle analysis certification|image CDN integration|backend integration/i);
    ...
    await page.screenshot({ path: `/tmp/news-performance-copy-budget-${isMobile ? "mobile" : "desktop"}-v1192.png`, fullPage: true });
  });
});
