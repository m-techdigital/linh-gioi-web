// v1.170 coverage: /guides/player-trust-release-guide prioritizes rendered browser UI layout and shared guide-flow Base First CSS.
...
async function collect(page: Page): Promise<Metrics> {
  return page.evaluate(() => {
    ...
    const hero = rect(".lgo-player-trust-release-guide-hero-card");
    ...
    return {
      ...
      firstFlowText: [
        document.querySelector(".lgo-player-trust-release-guide-hero-card")?.textContent ?? "",
...
test.describe("player trust release guide real UI layout v1.170", () => {
  test("/guides/player-trust-release-guide renders compact release-trust flow before generic CTAs", async ({ page, isMobile }, testInfo) => {
    await page.goto(`${web}/guides/player-trust-release-guide`);

    await expect(page.getByRole("heading", { level: 1, name: "Tin cậy trước khi chờ bản test" })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Các bước guide có kết quả mong đợi và phạm vi tạm khóa" })).toBeVisible();
    await expect(page.locator(".lgo-status-badge", { hasText: "Tin cậy phát hành" }).first()).toBeVisible();

    const metrics = await collect(page);
    expect(metrics.firstHeadings[0]).toEqual({ tag: "h1", text: "Tin cậy trước khi chờ bản test" });
    expect(metrics.firstFlowText).toContain("Đọc stage phát hành hiện tại");
    expect(metrics.firstFlowText).toContain("Kiểm chứng trước khi tin CTA tải");
    expect(metrics.firstFlowText).toContain("Xem trạng thái và ranh giới hỗ trợ");
    expect(metrics.firstFlowText).toContain("Theo dõi điều kiện closed test");
    expect(metrics.firstFlowText, "generic guide fallback and stale English labels must not drive the current page layout").not.toMatch(/WEB v1\.155|Guide tĩnh · chưa có hệ thống wiki|Player trust: từ content-ready tới closed test|proof-before-claim|support boundary|closed-test readiness|open beta|fake download button|placeholder checksum|launcher promise|open registration|automated entitlement/i);
    ...
    const trustLink = page.locator(".lgo-player-trust-release-guide-hero-card").getByRole("link", { name: "Tin cậy tải game" });
    ...
    if (isMobile) {
      ...
      expect(metrics.firstStepTop, "mobile first player-trust step appears near first fold").toBeLessThanOrEqual(760);
      expect(metrics.worldCtaTop, "mobile generic CTA starts after player-trust steps without a raw gap").toBeLessThanOrEqual(1290);
    ...
    const screenshotPath = `/tmp/guides-player-trust-release-${isMobile ? "mobile" : "desktop"}-v1170.png`;
    await page.screenshot({ path: screenshotPath, fullPage: true });
    await testInfo.attach(`guides-player-trust-release-${isMobile ? "mobile" : "desktop"}-v1170`, {
      path: screenshotPath,
