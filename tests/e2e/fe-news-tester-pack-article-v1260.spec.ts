...
const slug='closed-tester-information-pack-started',route=`/news/${slug}`;
...
test.describe('source-complete closed tester information article v1.260',()=>{
 ...
 test('reading layout and responsive contents replace compact proof cards',async({page,isMobile})=>{
  ...
  await page.screenshot({path:test.info().outputPath('news-tester-pack-article.png'),fullPage:true});
 });
 ...
 test('contents previous next and Back Forward move between the real sections',async({page})=>{
  const links=page.getByRole('navigation',{name:'Mục lục bài viết chuẩn bị kiểm thử'}).getByRole('link');await expect(links).toHaveCount(2);
 ...
 test('context links and breadcrumbs navigate to real published routes',async({page})=>{
  const links=page.locator('.lgo-news-article-action');await expect(links).toHaveCount(7);
  for(const [i,href] of ['/release/tester-pack#tester-checklist','/release/tester-pack#tester-device','/release/tester-pack#tester-limits','/release/tester-pack#tester-feedback','/guides/closed-tester-information-pack-guide','/support/safety','/support/help'].entries()){await expect(links.nth(i)).toHaveAttribute('href',href);expect((await page.request.get(origin+href)).status()).toBe(200);await links.nth(i).focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+href);await page.goBack();}
 ...
 test('reduced motion uses immediate native section arrival rather than smooth scrolling',async({page})=>{
  ...
  await page.getByRole('navigation',{name:'Mục lục bài viết chuẩn bị kiểm thử'}).getByRole('link').last().click();
 ...
 test('resizing the same article retains complete chapters and a single coherent contents navigation',async({page})=>{
  await page.emulateMedia({reducedMotion:'reduce'});
  const nav=page.getByRole('navigation',{name:'Mục lục bài viết chuẩn bị kiểm thử'});
 ...
 test('expanded text spacing does not clip authored text or hide reading links',async({page})=>{
  ...
  await root.locator('.lgo-news-article-action').last().focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+'/support/help');
 });

 for(const destination of ['tester-checklist','tester-device','tester-limits','tester-feedback']) {
  test(`article link reaches ${destination} with real focus and local-only controls`,async({page})=>{
   const href='/release/tester-pack#'+destination;
   const action=page.locator(`.lgo-news-article-action[href="${href}"]`);await expect(action).toHaveCount(1);
   await action.focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+href);
   const target=page.locator('#'+destination);await expect(target).toBeFocused();
   ...
   if(destination==='tester-checklist'){
    const checkbox=target.getByRole('checkbox').first();await expect(checkbox).toBeFocused();await page.keyboard.press('Space');await expect(checkbox).toBeChecked();
   }else if(destination==='tester-feedback'){
    await page.keyboard.press('End');await expect(target.getByRole('tab',{name:'Góp ý',exact:true})).toBeFocused();await expect(target.getByRole('tab',{name:'Góp ý',exact:true})).toHaveAttribute('aria-selected','true');
   }else{
    const summary=target.locator('summary').first();await expect(summary).toBeFocused();await page.keyboard.press('Enter');await expect(summary.locator('..')).toHaveAttribute('open','');
   }
   expect(requests).toEqual([]);await page.screenshot({path:test.info().outputPath(destination+'-from-article.png')});
   ...
   await page.goBack();await expect(page).toHaveURL(origin+route);await expect(page.getByRole('heading',{level:1,name:entry.title,exact:true})).toBeVisible();
  });
 }
