// v1.188 coverage: /news/community-roadmap-onboarding-started must render as compact Vietnamese community roadmap onboarding article, not English non-claim copy.
...
test.describe("news community roadmap onboarding real UI layout v1.188", () => {
  test("/news/community-roadmap-onboarding-started renders compact Vietnamese community roadmap onboarding article flow", async ({ page, isMobile }) => {
    await page.goto(`${web}/news/community-roadmap-onboarding-started`);

    await expect(page.getByRole("heading", { level: 1, name: "Onboarding cộng đồng và roadmap rõ hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích community onboarding và roadmap" })).toBeVisible();
    ...
    expect(mainText).not.toMatch(/Community \/ roadmap onboarding được làm rõ|No live community backend|fake waitlist|production account|No public game download artifact|portal entitlement|No CMS/i);
    ...
    await page.screenshot({ path: `/tmp/news-community-roadmap-${isMobile ? "mobile" : "desktop"}-v1188.png`, fullPage: true });
  });
});
