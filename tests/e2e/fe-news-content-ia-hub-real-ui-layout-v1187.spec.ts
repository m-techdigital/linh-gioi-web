// v1.187 coverage: /news/content-ia-hub-polish-started must render as compact Vietnamese content IA hub article with two detail cards, not a generic detail fallback.
...
test.describe("news content IA hub real UI layout v1.187", () => {
  test("/news/content-ia-hub-polish-started renders compact Vietnamese content IA hub article flow", async ({ page, isMobile }) => {
    await page.goto(`${web}/news/content-ia-hub-polish-started`);

    await expect(page.getByRole("heading", { level: 1, name: "Content hub và lộ trình đọc được làm rõ" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích content IA và hub bắt đầu" })).toBeVisible();
    ...
    expect(mainText).not.toMatch(/Bài viết giải thích nội dung public|No CMS|live personalization|account-aware recommendation backend|Runtime\/browser\/e2e vẫn chỉ là guardrail/i);
    ...
    await page.screenshot({ path: `/tmp/news-content-ia-hub-${isMobile ? "mobile" : "desktop"}-v1187.png`, fullPage: true });
  });
});
