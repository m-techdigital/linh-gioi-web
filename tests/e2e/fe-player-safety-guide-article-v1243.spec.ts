...
const route = '/guides/player-safety-support-guide';
const entry = contentEntries.find(item => item.slug === 'player-safety-support-guide')!;
const steps = guideDetailSteps.filter(item => item.slug === entry.slug);

test.describe('player safety preserves useful reading routes, without data intake v1.243', () => {
  ...
  test('four full article chapters and responsive contents replace compact proof boards', async ({ page, isMobile }) => {
    const guide = page.locator('.lgo-player-safety-guide');
    ...
    await page.screenshot({ path: test.info().outputPath('player-safety-article.png'), fullPage: true });
  });
  test('original summary body and all four instruction result boundary records are exact', async ({ page }) => {
    const guide = page.locator('.lgo-player-safety-guide'); await expect(guide).toBeVisible();
  ...
  test('native contents next previous and browser history follow the real chapter IDs', async ({ page }) => {
    const contents = page.getByRole('navigation', { name: 'Mục lục an toàn người chơi' });
    const links = contents.locator('a'); await expect(links).toHaveCount(4);
    await links.nth(1).focus(); await page.keyboard.press('Enter'); await expect(page.locator('#player-safety-step-02')).toBeFocused();
    await page.locator('#player-safety-step-02').getByRole('link', { name: /Phần tiếp/ }).click();
    await expect(page).toHaveURL(origin + route + '#player-safety-step-03'); await expect(page.locator('#player-safety-step-03')).toBeFocused();
    await page.goBack(); await expect(page).toHaveURL(origin + route + '#player-safety-step-02');
    await page.goForward(); await expect(page).toHaveURL(origin + route + '#player-safety-step-03');
    await page.locator('#player-safety-step-03').getByRole('link', { name: 'Về mục lục', exact: true }).click();
    await expect(page.locator('#player-safety-contents')).toBeFocused();
    const summary = page.locator('#player-safety-contents summary'); await summary.focus(); await page.keyboard.press('Space');
  ...
  test('cold direct URLs land visibly at the requested chapter and malformed fragments are harmless', async ({ page }) => {
    await page.goto('about:blank'); await page.goto(origin + route + '#player-safety-step-03', { waitUntil: 'networkidle' });
    const section = page.locator('#player-safety-step-03'); await expect(section).toBeFocused();
  ...
  test('chapter actions and related destinations navigate to existing guidance, not commands', async ({ page }) => {
    const actions = page.locator('.lgo-player-safety-action'); await expect(actions).toHaveCount(7);
    const routes = ['/support/help', '/support/safety', '/community', '/roadmap', '/status', '/download/trust', '/support'];
    ...
    await expect(actions.nth(1)).toHaveAttribute('href','/support/safety');
    await actions.last().click(); await expect(page).toHaveURL(origin + '/support');
  });
  test('source paper typography and actions remain legible at 320px with visible focus', async ({ page }) => {
    ...
    for (const link of await guide.locator('.lgo-article-section-navigation a,.lgo-player-safety-action').all()) expect((await link.boundingBox())!.height).toBeGreaterThanOrEqual(44);
    const action = guide.locator('.lgo-player-safety-action').first(); await action.focus(); await expect(action).toBeFocused();
  ...
  test('reading has no submission saved progress access award or gameplay surrogate', async ({ page }) => {
    const guide = page.locator('.lgo-player-safety-guide'); await expect(guide).toBeVisible();
    await expect(guide).toContainText('Hướng dẫn chuẩn bị, không nhận báo lỗi hoặc dữ liệu riêng tư'); await expect(guide).toContainText('NO_ACCEPTED_BACKEND_CONTRACT');
    ...
    await page.getByRole('navigation', { name: 'Mục lục an toàn người chơi' }).locator('a').first().click();
    await page.locator('#player-safety-step-01').getByRole('link', { name: /Phần tiếp/ }).click();
    expect(requests).toEqual([]); await expect(page.locator('main h1')).toHaveCount(1);
    await expect(guide.locator('.lgo-article-boundary').last()).toContainText('Chưa có live support ticket');
    await expect(guide.locator('[role=timer],[data-countdown]')).toHaveCount(0);
    await expect(guide.getByRole('button', { name: /Đăng ký|Tham gia|Nhận quyền/i })).toHaveCount(0);
  ...
  test('community and status choices preserve every authored destination and keyboard order', async ({ page }) => {
    const community=page.getByRole('navigation',{name:'Các trang định hướng cộng đồng',exact:true});
    const status=page.getByRole('navigation',{name:'Đối chiếu trước khi chờ hỗ trợ',exact:true});
    await expect(community.getByRole('link')).toHaveCount(2);await expect(status.getByRole('link')).toHaveCount(3);
    const routes=[['/community','/roadmap'],['/status','/download/trust','/support']];
    for(const [i,group] of [community,status].entries()){
      for(const [j,link] of (await group.getByRole('link').all()).entries()){
        await expect(link).toHaveAttribute('href',routes[i]![j]!);await link.focus();await expect(link).toBeFocused();
        expect(await link.evaluate(e=>getComputedStyle(e).outlineStyle)).not.toBe('none');
        expect(await link.evaluate(e=>getComputedStyle(e).backgroundColor)).toBe('rgb(7, 19, 28)');
      }
    }
    await status.getByRole('link').first().focus();await page.keyboard.press('Tab');await expect(status.getByRole('link').nth(1)).toBeFocused();
    await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+'/download/trust');
    await page.goBack();await expect(status).toBeVisible();
    await page.setViewportSize({width:320,height:800});
    for(const a of await page.locator('.lgo-player-safety-action').all())expect((await a.boundingBox())!.height).toBeGreaterThanOrEqual(44);
    expect(await page.evaluate(()=>document.documentElement.scrollWidth-innerWidth)).toBeLessThanOrEqual(0);
    await page.screenshot({path:test.info().outputPath('safe-reading-choices-320.png'),fullPage:true});
  });
