...
import {contentEntries,contentDetailSections} from '../../packages/content/src/fixtures';
const origin=process.env.LGO_WEB_URL??'http://127.0.0.1:3000';
const slug='content-ia-hub-polish-started',route=`/news/${slug}`;
...
test.describe('source-complete content hub reading article v1.262',()=>{
 ...
 test('reading layout and responsive contents replace compact proof cards',async({page,isMobile})=>{
  ...
  await page.screenshot({path:test.info().outputPath('news-content-hub-article.png'),fullPage:true});
 });
 ...
 test('contents previous next and Back Forward move between the real sections',async({page})=>{
  const links=page.getByRole('navigation',{name:'Mục lục bài viết tìm đường đọc'}).getByRole('link');await expect(links).toHaveCount(2);
 ...
 test('reduced motion uses immediate native section arrival rather than smooth scrolling',async({page})=>{
  ...
  await page.getByRole('navigation',{name:'Mục lục bài viết tìm đường đọc'}).getByRole('link').last().click();
 ...
 test('resizing the same article retains complete chapters and a single coherent contents navigation',async({page})=>{
  await page.emulateMedia({reducedMotion:'reduce'});
  const nav=page.getByRole('navigation',{name:'Mục lục bài viết tìm đường đọc'});
 ...
 test('expanded text spacing does not clip authored text or hide reading links',async({page})=>{
  ...
  await root.locator('.lgo-news-article-action').last().focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+'/community');
 });

 test('the guide-library destination supports topic and keyword filtering then opens the authored hub guide',async({page})=>{
  const guide=contentEntries.find(e=>e.slug==='start-here-content-hub-guide')!;
  const allGuides=contentEntries.filter(e=>e.category==='guides'&&e.status==='published');
  await page.locator('.lgo-news-article-action[href="/guides"]').click();await expect(page).toHaveURL(origin+'/guides');
  const catalog=page.locator('.lgo-reading-catalog'),input=catalog.getByRole('searchbox',{name:'Tìm trong thư viện'});
  await expect(catalog.locator('article')).toHaveCount(allGuides.length);await page.waitForLoadState('networkidle');
  const requests:string[]=[];page.on('request',r=>{if(r.method()!=='GET'||['fetch','xhr'].includes(r.resourceType()))requests.push(r.url());});
  const storage=()=>page.evaluate(()=>({local:{...localStorage},session:{...sessionStorage}}));const before=await storage();
  const group=catalog.getByRole('button',{name:/^Đọc website/});await group.focus();await page.keyboard.press('Space');await expect(group).toBeFocused();await expect(group).toHaveAttribute('aria-pressed','true');
  const query=guide.title.normalize('NFD').replace(/\p{M}/gu,'').replace(/[đĐ]/g,'d').toUpperCase();await input.fill('  '+query+'  ');
  const card=catalog.locator('article');await expect(card).toHaveCount(1);await expect(card).toHaveAttribute('data-entry-id',guide.slug);await expect(catalog.getByRole('status')).toContainText(`1/${allGuides.length}`);
  await card.locator('summary').focus();await page.keyboard.press('Enter');await expect(card.locator('details p')).toHaveText(guide.summary);expect(requests).toEqual([]);expect(await storage()).toEqual(before);
  await page.screenshot({path:test.info().outputPath('guide-filter-from-news.png')});await card.getByRole('link').click();await expect(page.getByRole('heading',{level:1,name:guide.title,exact:true})).toBeVisible();
  await page.goBack();await expect(page).toHaveURL(origin+'/guides');await page.reload();await expect(input).toHaveValue('');await expect(catalog.locator('article')).toHaveCount(allGuides.length);await page.goBack();await expect(page).toHaveURL(origin+route);
 });
 test('the news-library destination can find and reopen this article without persisting search text',async({page})=>{
  await page.locator('.lgo-news-article-action[href="/news"]').click();await expect(page).toHaveURL(origin+'/news');
  const catalog=page.locator('.lgo-reading-catalog'),input=catalog.getByRole('searchbox',{name:'Tìm trong bản tin'});
  const published=contentEntries.filter(e=>e.category==='news'&&e.status==='published');await expect(catalog.locator('article')).toHaveCount(published.length);await page.waitForLoadState('networkidle');
  const requests:string[]=[];page.on('request',r=>{if(r.method()!=='GET'||['fetch','xhr'].includes(r.resourceType()))requests.push(r.url());});
  await input.fill(entry.title);const card=catalog.locator('article');await expect(card).toHaveCount(1);await expect(card).toHaveAttribute('data-entry-id',slug);await card.locator('summary').click();await expect(card.locator('details p')).toHaveText(entry.summary);expect(requests).toEqual([]);
  await page.screenshot({path:test.info().outputPath('news-hub-filter.png')});await card.getByRole('link').click();await expect(page).toHaveURL(origin+route);await expect(page.locator('.lgo-news-article')).toBeVisible();await page.goBack();await page.reload();await expect(input).toHaveValue('');await expect(catalog.locator('article')).toHaveCount(published.length);
 });
 test('the Start destination retains actual reading routes rather than offering account or game access',async({page})=>{
  await page.locator('.lgo-news-article-action[href="/start"]').focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+'/start');
  const start=page.locator('.lgo-startpage-stack');await expect(start.getByRole('heading',{level:1,name:'Bắt đầu',exact:true})).toBeVisible();await expect(start.locator('.lgo-onboarding-steps span')).toHaveCount(6);
  const action=start.getByRole('link',{name:'Tìm hiểu năm Lộ',exact:true});await expect(action).toHaveAttribute('href','/classes');await action.focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+'/classes');await expect(page.locator('main h1')).toBeVisible();await page.goBack();await expect(page).toHaveURL(origin+'/start');
  await expect(start.locator('form,input[type=password],a[download]')).toHaveCount(0);await expect(start).toContainText('không mở tải game hay backend tài khoản');await page.goBack();await expect(page).toHaveURL(origin+route);
 });
