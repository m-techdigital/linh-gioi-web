// v1.190 coverage: /news/player-safety-support-faq-polish-started must render compact Vietnamese player safety support article with detail cards, not a body-only support fixture.
...
test.describe("news player safety support FAQ real UI layout v1.190", () => {
  test("/news/player-safety-support-faq-polish-started renders compact Vietnamese player safety support article flow", async ({ page, isMobile }) => {
    await page.goto(`${web}/news/player-safety-support-faq-polish-started`);

    await expect(page.getByRole("heading", { level: 1, name: "FAQ an toàn và hỗ trợ rõ hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích an toàn người chơi và FAQ hỗ trợ" })).toBeVisible();
    ...
    expect(mainText).not.toMatch(/Bài viết giải thích nội dung public|ticket live|moderation dashboard|backend integration|tra cứu tài khoản|claim ticket live/i);
    ...
    await page.screenshot({ path: `/tmp/news-player-safety-support-${isMobile ? "mobile" : "desktop"}-v1190.png`, fullPage: true });
  });
});
