...
const slug='visual-responsive-polish-started',route=`/news/${slug}`;
...
test.describe('source-complete visual responsive article v1.256',()=>{
 ...
 test('reading layout and responsive contents replace compact proof cards',async({page,isMobile})=>{
  ...
  await page.screenshot({path:test.info().outputPath('news-visual-responsive-article.png'),fullPage:true});
 });
 ...
 test('contents previous next and Back Forward move between the real sections',async({page})=>{
  const links=page.getByRole('navigation',{name:'Mục lục bài viết visual và responsive'}).getByRole('link');await expect(links).toHaveCount(2);
 ...
 test('context links and breadcrumbs navigate to real published routes',async({page})=>{
  const links=page.locator('.lgo-news-article-action');await expect(links).toHaveCount(6);
  for(const [i,href] of ['/game','/download','/support/help','/accessibility','/performance','/news'].entries()){await expect(links.nth(i)).toHaveAttribute('href',href);expect((await page.request.get(origin+href)).status()).toBe(200);await links.nth(i).focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+href);await page.goBack();}
 ...
 test('reduced motion uses immediate native section arrival rather than smooth scrolling',async({page})=>{
  ...
  await page.getByRole('navigation',{name:'Mục lục bài viết visual và responsive'}).getByRole('link').last().click();
 ...
 test('resizing the same article retains complete chapters and a single coherent contents navigation',async({page})=>{
  await page.emulateMedia({reducedMotion:'reduce'});
  const nav=page.getByRole('navigation',{name:'Mục lục bài viết visual và responsive'});
 ...
 test('expanded text spacing does not clip authored text or hide reading links',async({page})=>{
  ...
  await root.locator('.lgo-news-article-action').last().focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+'/news');
 });
