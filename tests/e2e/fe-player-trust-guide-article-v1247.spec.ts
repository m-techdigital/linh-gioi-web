...
const route = '/guides/player-trust-release-guide';
const entry = contentEntries.find(item => item.slug === 'player-trust-release-guide')!;
const steps = guideDetailSteps.filter(item => item.slug === entry.slug);

test.describe('player trust explains release conditions, not test admission v1.247', () => {
  ...
  test('four full article chapters and responsive contents replace compact proof boards', async ({ page, isMobile }) => {
    const guide = page.locator('.lgo-player-trust-guide');
    ...
    await page.screenshot({ path: test.info().outputPath('player-trust-article.png'), fullPage: true });
  });
  test('original summary body and all four instruction result boundary records are exact', async ({ page }) => {
    const guide = page.locator('.lgo-player-trust-guide'); await expect(guide).toBeVisible();
  ...
  test('native contents next previous and browser history follow the real chapter IDs', async ({ page }) => {
    const contents = page.getByRole('navigation', { name: 'Mục lục tin cậy trước bản test' });
    const links = contents.locator('a'); await expect(links).toHaveCount(4);
    await links.nth(1).focus(); await page.keyboard.press('Enter'); await expect(page.locator('#player-trust-step-02')).toBeFocused();
    await page.locator('#player-trust-step-02').getByRole('link', { name: /Phần tiếp/ }).click();
    await expect(page).toHaveURL(origin + route + '#player-trust-step-03'); await expect(page.locator('#player-trust-step-03')).toBeFocused();
    await page.goBack(); await expect(page).toHaveURL(origin + route + '#player-trust-step-02');
    await page.goForward(); await expect(page).toHaveURL(origin + route + '#player-trust-step-03');
    await page.locator('#player-trust-step-03').getByRole('link', { name: 'Về mục lục', exact: true }).click();
    await expect(page.locator('#player-trust-contents')).toBeFocused();
    const summary = page.locator('#player-trust-contents summary'); await summary.focus(); await page.keyboard.press('Space');
  ...
  test('cold direct URLs land visibly at the requested chapter and malformed fragments are harmless', async ({ page }) => {
    await page.goto('about:blank'); await page.goto(origin + route + '#player-trust-step-03', { waitUntil: 'networkidle' });
    const section = page.locator('#player-trust-step-03'); await expect(section).toBeFocused();
  ...
  test('chapter actions and related destinations navigate to existing guidance, not commands', async ({ page }) => {
    const actions = page.locator('.lgo-player-trust-action'); await expect(actions).toHaveCount(6);
    const routes = ['/release', '/download/trust', '/status', '/support/safety', '/release/tester-pack#tester-checklist', '/release/readiness'];
    ...
    await expect(actions.nth(1)).toHaveAttribute('href','/download/trust');
    await actions.last().click(); await expect(page).toHaveURL(origin + '/release/readiness');
  });
  test('source paper typography and actions remain legible at 320px with visible focus', async ({ page }) => {
    ...
    for (const link of await guide.locator('.lgo-article-section-navigation a,.lgo-player-trust-action').all()) expect((await link.boundingBox())!.height).toBeGreaterThanOrEqual(44);
    const action = guide.locator('.lgo-player-trust-action').first(); await action.focus(); await expect(action).toBeFocused();
  ...
  test('reading has no submission saved progress access award or gameplay surrogate', async ({ page }) => {
    const guide = page.locator('.lgo-player-trust-guide'); await expect(guide).toBeVisible();
    await expect(guide).toContainText('Đọc hướng dẫn không cấp suất test hoặc quyền tải'); await expect(guide).toContainText('NO_ACCEPTED_BACKEND_CONTRACT');
    ...
    await page.getByRole('navigation', { name: 'Mục lục tin cậy trước bản test' }).locator('a').first().click();
    await page.locator('#player-trust-step-01').getByRole('link', { name: /Phần tiếp/ }).click();
    expect(requests).toEqual([]); await expect(page.locator('main h1')).toHaveCount(1);
    await expect(guide.locator('.lgo-article-boundary').nth(1)).toContainText('Chưa có nút tải thật, checksum thật');
    await expect(guide.locator('.lgo-article-boundary').last()).toContainText('Chưa có đăng ký mở, đảm bảo suất test');
  ...
  test('source-directed status and test-preparation groups keep keyboard order and real destinations', async ({ page }) => {
    const status = page.getByRole('navigation', { name: 'Đọc trạng thái và chuẩn bị phản hồi', exact: true });
    const links = status.getByRole('link'); await expect(links).toHaveCount(2);
    ...
    await page.goBack();
    const preparation = page.getByRole('navigation', { name: 'Đọc checklist và điều kiện thử nghiệm', exact: true });
    await expect(preparation.getByRole('link')).toHaveCount(2);
    await preparation.getByRole('link', { name: /Mở checklist chuẩn bị/ }).click();
    await expect(page).toHaveURL(origin + '/release/tester-pack#tester-checklist');
    const checklist = page.locator('#tester-checklist'); await expect(checklist).toBeVisible();
    await expect(checklist.locator('input:checked')).toHaveCount(0);
    await expect(checklist.getByRole('checkbox').first()).toBeVisible();
    expect(await page.evaluate(() => document.documentElement.scrollWidth - innerWidth)).toBeLessThanOrEqual(0);
  });
  test('illustration is not release evidence and losing it cannot hide source guidance', async ({ page }) => {
    ...
    await expect(guide.locator('.lgo-article-cover figcaption')).toContainText('Không phải bản test');
    await expect(guide.locator('.lgo-guide-article-section')).toHaveCount(4);
    await expect(guide.locator('.lgo-hero-lead')).toHaveText(entry.summary);
    await page.getByRole('navigation', { name: 'Mục lục tin cậy trước bản test' }).locator('a').last().click();
    await expect(page.locator('#player-trust-step-04')).toBeFocused();
    await expect(page.locator('#player-trust-step-04 .lgo-article-instruction')).toHaveText(steps[3]!.action);
