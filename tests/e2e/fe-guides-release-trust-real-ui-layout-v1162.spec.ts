// v1.162 coverage: /guides/release-trust-and-checksum-guide prioritizes rendered browser UI layout and shared guide-flow Base First CSS.
...
async function collect(page: Page): Promise<Metrics> {
  return page.evaluate(() => {
    ...
    const hero = rect(".lgo-release-trust-hero-card");
    ...
    return {
      ...
      firstFlowText: [
        document.querySelector(".lgo-release-trust-hero-card")?.textContent ?? "",
...
test.describe("release trust checksum guide real UI layout v1.162", () => {
  test("/guides/release-trust-and-checksum-guide renders artifact/checksum proof flow before generic CTAs", async ({ page, isMobile }, testInfo) => {
    await page.goto(`${web}/guides/release-trust-and-checksum-guide`);

    await expect(page.getByRole("heading", { level: 1, name: "Tin cậy phát hành và checksum" })).toBeVisible();
    ...
    expect(metrics.firstHeadings[0]).toEqual({ tag: "h1", text: "Tin cậy phát hành và checksum" });
    expect(metrics.firstFlowText).toContain("gói build");
    expect(metrics.firstFlowText).toContain("checksum");
    expect(metrics.firstFlowText).toContain("phê duyệt chủ sở hữu");
    expect(metrics.firstFlowText, "placeholder English must not drive the current page layout").not.toMatch(/Release trust and checksum guide|A player-facing guide|This guide turns release trust|No public game download artifact|No placeholder checksum|WEB v1\.155/);
    ...
    const trustLink = page.locator(".lgo-release-trust-hero-card").getByRole("link", { name: "Tin cậy tải game" });
    ...
    if (isMobile) {
      ...
      expect(metrics.firstStepTop, "mobile first proof step appears in first fold").toBeLessThanOrEqual(790);
    ...
    const screenshotPath = `/tmp/guides-release-trust-${isMobile ? "mobile" : "desktop"}-v1162.png`;
    await page.screenshot({ path: screenshotPath, fullPage: true });
    await testInfo.attach(`guides-release-trust-${isMobile ? "mobile" : "desktop"}-v1162`, {
      path: screenshotPath,
