// v1.178 coverage: /news/public-ux-content-polish-started must render as a compact Vietnamese UX/content article detail, not a raw WEB v1.6 fixture body.
...
test.describe("news public UX content real UI layout v1.178", () => {
  test("/news/public-ux-content-polish-started renders compact Vietnamese UX article detail flow", async ({ page, isMobile }) => {
    await page.goto(`${web}/news/public-ux-content-polish-started`);

    await expect(page.getByRole("heading", { level: 1, name: "Bắt đầu polish UX và nội dung public" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích polish UX public" })).toBeVisible();
    ...
    expect(mainText).not.toMatch(/WEB v1\.6 content polish established|player-facing foundation while keeping non-claims explicit|no production auth, no DB persistence, no CMS and no real backend integration|WEB v1\.9 article detail|Boundary:|PROVISIONAL_WEB_FIXTURE|Detail section|This public content is/i);
    ...
    await page.screenshot({ path: `/tmp/news-public-ux-${isMobile ? "mobile" : "desktop"}-v1178.png`, fullPage: true });
  });
});
