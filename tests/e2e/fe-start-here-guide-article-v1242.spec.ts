...
const route = '/guides/start-here-content-hub-guide';
const entry = contentEntries.find(item => item.slug === 'start-here-content-hub-guide')!;
const steps = guideDetailSteps.filter(item => item.slug === entry.slug);

test.describe('start-here guide has useful reading choices, not personalization v1.242', () => {
  ...
  test('four full article chapters and responsive contents replace compact proof boards', async ({ page, isMobile }) => {
    const guide = page.locator('.lgo-start-here-guide');
    ...
    await page.screenshot({ path: test.info().outputPath('start-here-article.png'), fullPage: true });
  });
  test('original summary body and all four instruction result boundary records are exact', async ({ page }) => {
    const guide = page.locator('.lgo-start-here-guide'); await expect(guide).toBeVisible();
  ...
  test('native contents next previous and browser history follow the real chapter IDs', async ({ page }) => {
    const contents = page.getByRole('navigation', { name: 'Mục lục bắt đầu đọc web' });
    const links = contents.locator('a'); await expect(links).toHaveCount(4);
    await links.nth(1).focus(); await page.keyboard.press('Enter'); await expect(page.locator('#start-here-step-02')).toBeFocused();
    await page.locator('#start-here-step-02').getByRole('link', { name: /Phần tiếp/ }).click();
    await expect(page).toHaveURL(origin + route + '#start-here-step-03'); await expect(page.locator('#start-here-step-03')).toBeFocused();
    await page.goBack(); await expect(page).toHaveURL(origin + route + '#start-here-step-02');
    await page.goForward(); await expect(page).toHaveURL(origin + route + '#start-here-step-03');
    await page.locator('#start-here-step-03').getByRole('link', { name: 'Về mục lục', exact: true }).click();
    await expect(page.locator('#start-here-contents')).toBeFocused();
    const summary = page.locator('#start-here-contents summary'); await summary.focus(); await page.keyboard.press('Space');
  ...
  test('cold direct URLs land visibly at the requested chapter and malformed fragments are harmless', async ({ page }) => {
    await page.goto('about:blank'); await page.goto(origin + route + '#start-here-step-03', { waitUntil: 'networkidle' });
    const section = page.locator('#start-here-step-03'); await expect(section).toBeFocused();
  ...
  test('chapter actions and related destinations navigate to existing guidance, not commands', async ({ page }) => {
    const actions = page.locator('.lgo-start-here-action'); await expect(actions).toHaveCount(6);
    const routes = ['/start', '/game', '/download/trust', '/roadmap', '/release/readiness', '/support/help'];
    ...
    await expect(actions.nth(1)).toHaveAttribute('href','/game');
    await actions.last().click(); await expect(page).toHaveURL(origin + '/support/help');
  });
  test('source paper typography and actions remain legible at 320px with visible focus', async ({ page }) => {
    ...
    for (const link of await guide.locator('.lgo-article-section-navigation a,.lgo-start-here-action').all()) expect((await link.boundingBox())!.height).toBeGreaterThanOrEqual(44);
    const action = guide.locator('.lgo-start-here-action').first(); await action.focus(); await expect(action).toBeFocused();
  ...
  test('reading has no submission saved progress access award or gameplay surrogate', async ({ page }) => {
    const guide = page.locator('.lgo-start-here-guide'); await expect(guide).toBeVisible();
    await expect(guide).toContainText('Chỉ chọn đường đọc, không tạo tài khoản hoặc lưu sở thích'); await expect(guide).toContainText('NO_ACCEPTED_BACKEND_CONTRACT');
    ...
    await page.getByRole('navigation', { name: 'Mục lục bắt đầu đọc web' }).locator('a').first().click();
    await page.locator('#start-here-step-01').getByRole('link', { name: /Phần tiếp/ }).click();
    expect(requests).toEqual([]); await expect(page.locator('main h1')).toHaveCount(1);
    await expect(guide.locator('.lgo-article-boundary').last()).toContainText('Chưa có nút tải thật');
  ...
  test('the authored choice of three reading directions is a keyboard-usable shared group', async ({ page }) => {
    const group=page.getByRole('navigation',{name:'Chọn nhóm trang phù hợp',exact:true});
    await expect(group).toBeVisible();const links=group.getByRole('link');await expect(links).toHaveCount(3);
    const destinations=['/game','/download/trust','/roadmap'];
    for(const [i,a] of (await links.all()).entries()){
      await expect(a).toHaveAttribute('href',destinations[i]!);
      await a.focus();await expect(a).toBeFocused();
      expect(await a.evaluate(e=>getComputedStyle(e).outlineStyle)).not.toBe('none');
      expect(await a.evaluate(e=>getComputedStyle(e).backgroundColor)).toBe('rgb(7, 19, 28)');
    }
    ...
    expect(await page.evaluate(()=>document.documentElement.scrollWidth-innerWidth)).toBeLessThanOrEqual(0);
    await page.screenshot({path:test.info().outputPath('reading-choices-320.png'),fullPage:true});
  });
