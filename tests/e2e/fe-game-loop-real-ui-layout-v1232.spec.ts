...
test.describe('world loop reading journey, not gameplay v1.232',()=>{
 ...
 test('world illustration and four real reading choices replace a static game diagram',async({page,isMobile})=>{
  ...
  const rail=await page.locator('#loop-reading .lgo-progress-steps').evaluate(el=>{const n=el as HTMLElement,s=getComputedStyle(n),boxes=[...n.children].map(c=>(c as HTMLElement).getBoundingClientRect());return{display:s.display,overflowX:s.overflowX,rows:new Set(boxes.map(r=>Math.round(r.top))).size,scrollWidth:n.scrollWidth,clientWidth:n.clientWidth};});
  if(isMobile){expect(m.vista.top).toBeGreaterThanOrEqual(m.copy.bottom);expect(rail.display).toBe('flex');expect(rail.overflowX).toBe('auto');expect(rail.rows).toBe(1);expect(rail.scrollWidth).toBeGreaterThan(rail.clientWidth);}else{expect(m.vista.left).toBeGreaterThanOrEqual(m.copy.right);expect(new Set(m.steps.map(s=>Math.round(s.top))).size).toBe(1);}
