...
const slug='public-ux-content-polish-started',route=`/news/${slug}`;
...
test.describe('source-complete public UX article v1.255',()=>{
 ...
 test('reading layout and responsive contents replace compact proof cards',async({page,isMobile})=>{
  ...
  await page.screenshot({path:test.info().outputPath('news-public-ux-article.png'),fullPage:true});
 });
 ...
 test('contents previous next and Back Forward move between the real sections',async({page})=>{
  const links=page.getByRole('navigation',{name:'Mục lục bài viết UX public'}).getByRole('link');await expect(links).toHaveCount(2);
 ...
 test('context links and breadcrumbs navigate to real published routes',async({page})=>{
  ...
  for(const [i,href] of ['/','/game','/roadmap','/download','/support/help','/news'].entries()){await expect(links.nth(i)).toHaveAttribute('href',href);expect((await page.request.get(origin+href)).status()).toBe(200);await links.nth(i).focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+href);await page.goBack();}
 ...
 test('reduced motion uses immediate native section arrival rather than smooth scrolling',async({page})=>{
  ...
  await page.getByRole('navigation',{name:'Mục lục bài viết UX public'}).getByRole('link').last().click();
