// v1.182 coverage: /news/status-download-trust-polish-started must render as compact Vietnamese status/download trust article, not a raw WEB v1.10 fixture body.
...
test.describe("news status download trust real UI layout v1.182", () => {
  test("/news/status-download-trust-polish-started renders compact Vietnamese status download trust article flow", async ({ page, isMobile }) => {
    await page.goto(`${web}/news/status-download-trust-polish-started`);

    await expect(page.getByRole("heading", { level: 1, name: "Copy trạng thái và download trust rõ hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích status và download trust" })).toBeVisible();
    ...
    expect(mainText).not.toMatch(/WEB v1\.10 focuses on release trust wording|WEB v1\.10 keeps building the public web product|public game artifacts, production auth, DB persistence|No public game download artifact|no entitlement backend|Runtime\/browser\/e2e is guardrail only|not public release readiness|checksum\/provenance explanation|support expectations without adding backend claims/i);
    ...
    await page.screenshot({ path: `/tmp/news-status-download-trust-${isMobile ? "mobile" : "desktop"}-v1182.png`, fullPage: true });
  });
});
