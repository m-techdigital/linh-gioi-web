...
test.describe('patch-notes are readable announcements, not live operations v1.252',()=>{
  test.beforeEach(async({context,page})=>{await mountStaticBuild(context);await page.goto(origin+'/patch-notes');await expect(page.getByRole('heading',{level:1,name:'Ghi chú cập nhật Linh Giới',exact:true})).toBeVisible();});
  test('illustrated hero and full-width editorial announcement replace tiny proof cards',async({page,isMobile})=>{
    ...
    await root.locator('#patch-notes-web-archive > summary').click();
  ...
  test('only published development records appear, with exact descriptions and posting time not game release dates',async({page})=>{
    await page.locator('#patch-notes-web-archive > summary').click();
  ...
  test('announcement and boundary links arrive below the header and keep native history',async({page})=>{
    await page.getByRole('link',{name:'Xem trạng thái cập nhật',exact:true}).click();
    ...
    await page.keyboard.press('Tab');await expect(page.locator('.lgo-patch-notes-reading-routes a').first()).toBeFocused();
  ...
  test('no version checksum update download or production release is manufactured',async({page})=>{
    ...
    await root.locator('#patch-notes-web-archive > summary').click();await root.locator('.lgo-announcement-card summary').first().click();await expect(root.locator('.lgo-announcement-body').first()).toHaveText(entries[0]!.body);expect(errors).toEqual([]);
  });
