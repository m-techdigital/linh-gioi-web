...
test.describe("news player trust release real UI layout v1.194", () => {
  test("/news/player-trust-release-narrative-started renders compact Vietnamese player trust release article flow", async ({ page, isMobile }) => {
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Niềm tin phát hành rõ ràng hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích niềm tin người chơi và lộ trình phát hành" })).toBeVisible();
