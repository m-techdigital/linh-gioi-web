// v1.169 coverage: /guides/route-continuity-conversion-guide prioritizes rendered browser UI layout and shared guide-flow Base First CSS.
...
async function collect(page: Page): Promise<Metrics> {
  return page.evaluate(() => {
    ...
    const hero = rect(".lgo-route-continuity-guide-hero-card");
    ...
    return {
      ...
      firstFlowText: [
        document.querySelector(".lgo-route-continuity-guide-hero-card")?.textContent ?? "",
...
test.describe("route continuity conversion guide real UI layout v1.169", () => {
  test("/guides/route-continuity-conversion-guide renders compact reading-route flow before generic CTAs", async ({ page, isMobile }, testInfo) => {
    await page.goto(`${web}/guides/route-continuity-conversion-guide`);

    await expect(page.getByRole("heading", { level: 1, name: "Đi tiếp đúng luồng đọc" })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Các bước guide có kết quả mong đợi và phạm vi tạm khóa" })).toBeVisible();
    await expect(page.locator(".lgo-status-badge", { hasText: "Luồng đọc an toàn" }).first()).toBeVisible();

    const metrics = await collect(page);
    expect(metrics.firstHeadings[0]).toEqual({ tag: "h1", text: "Đi tiếp đúng luồng đọc" });
    expect(metrics.firstFlowText).toContain("Bắt đầu từ câu hỏi người chơi");
    expect(metrics.firstFlowText).toContain("Đi qua vòng lặp thế giới");
    expect(metrics.firstFlowText).toContain("Kiểm gate tải game và trạng thái");
    expect(metrics.firstFlowText).toContain("Đóng vòng bằng hỗ trợ an toàn");
    expect(metrics.firstFlowText, "generic guide fallback and stale English labels must not drive the current page layout").not.toMatch(/WEB v1\.155|Guide tĩnh · chưa có hệ thống wiki|Route continuity and safe conversion guide|World Loop|Download Trust|Support Safety|personalized recommendation|secure inbox|conversion-safe|fake download|account-aware|public game artifact/i);
    ...
    const trustLink = page.locator(".lgo-route-continuity-guide-hero-card").getByRole("link", { name: "Tin cậy tải game" });
    ...
    if (isMobile) {
      ...
      expect(metrics.firstStepTop, "mobile first route-continuity step appears near first fold").toBeLessThanOrEqual(760);
      expect(metrics.worldCtaTop, "mobile generic CTA starts after route-continuity steps without a raw gap").toBeLessThanOrEqual(1290);
    ...
    const screenshotPath = `/tmp/guides-route-continuity-conversion-${isMobile ? "mobile" : "desktop"}-v1169.png`;
    await page.screenshot({ path: screenshotPath, fullPage: true });
    await testInfo.attach(`guides-route-continuity-conversion-${isMobile ? "mobile" : "desktop"}-v1169`, {
      path: screenshotPath,
