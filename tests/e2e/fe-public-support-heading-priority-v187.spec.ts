// v1.87 coverage: support/help routes start main content with the page h1 before CTA sections.
import { test, expect, type Page } from "@playwright/test";

const web = process.env.LGO_WEB_URL ?? "http://127.0.0.1:3000";
const routes = [
  { path: "/support", title: "Hỗ trợ cộng đồng" },
  { path: "/support/help", title: "FAQ nhanh" },
  { path: "/support/safety", title: "Báo lỗi an toàn" },
];
...
test.describe("public support heading priority", () => {
  for (const route of routes) {
    test(`${route.path} starts support content with the page h1`, async ({ page, isMobile }) => {
