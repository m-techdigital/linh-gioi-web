...
const slug='web-program-control-tower',route=`/news/${slug}`;
...
test.describe('source-complete control tower article v1.254',()=>{
 ...
 test('reading layout and responsive contents replace compact proof cards',async({page,isMobile})=>{
  ...
  await page.screenshot({path:test.info().outputPath('news-control-article.png'),fullPage:true});
 });
 ...
 test('contents previous next and Back Forward move between the real sections',async({page})=>{
  const links=page.getByRole('navigation',{name:'Mục lục bài viết Control tower'}).getByRole('link');await expect(links).toHaveCount(2);
 ...
 test('context links and breadcrumbs navigate to real published routes',async({page})=>{
  const links=page.locator('.lgo-news-article-action');await expect(links).toHaveCount(4);
  for(const [i,href] of ['/status','/release','/guides','/news'].entries()){await expect(links.nth(i)).toHaveAttribute('href',href);expect((await page.request.get(origin+href)).status()).toBe(200);await links.nth(i).focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+href);await page.goBack();}
 ...
 test('reduced motion uses immediate native section arrival rather than smooth scrolling',async({page})=>{
  ...
  await page.getByRole('navigation',{name:'Mục lục bài viết Control tower'}).getByRole('link').last().click();
