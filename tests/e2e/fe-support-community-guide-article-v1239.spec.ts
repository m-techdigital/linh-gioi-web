...
const route = '/guides/support-and-community-guide';
const entry = contentEntries.find(item => item.slug === 'support-and-community-guide')!;
const steps = guideDetailSteps.filter(item => item.slug === entry.slug);

test.describe('support and community guide keeps reading separate from live support v1.239', () => {
  ...
  test('four full article chapters and responsive contents replace compact proof boards', async ({ page, isMobile }) => {
    const guide = page.locator('.lgo-support-community-guide');
    ...
    await page.screenshot({ path: test.info().outputPath('support-community-article.png'), fullPage: true });
  });
  test('original summary body and all four instruction result boundary records are exact', async ({ page }) => {
    const guide = page.locator('.lgo-support-community-guide'); await expect(guide).toBeVisible();
  ...
  test('native contents next previous and browser history follow the real chapter IDs', async ({ page }) => {
    const contents = page.getByRole('navigation', { name: 'Mục lục hỗ trợ và cộng đồng' });
    const links = contents.locator('a'); await expect(links).toHaveCount(4);
    await links.nth(1).focus(); await page.keyboard.press('Enter'); await expect(page.locator('#support-community-step-02')).toBeFocused();
    await page.locator('#support-community-step-02').getByRole('link', { name: /Phần tiếp/ }).click();
    await expect(page).toHaveURL(origin + route + '#support-community-step-03'); await expect(page.locator('#support-community-step-03')).toBeFocused();
    await page.goBack(); await expect(page).toHaveURL(origin + route + '#support-community-step-02');
    await page.goForward(); await expect(page).toHaveURL(origin + route + '#support-community-step-03');
    await page.locator('#support-community-step-03').getByRole('link', { name: 'Về mục lục', exact: true }).click();
    await expect(page.locator('#support-community-contents')).toBeFocused();
    const summary = page.locator('#support-community-contents summary'); await summary.focus(); await page.keyboard.press('Space');
  ...
  test('cold direct URLs land visibly at the requested chapter and malformed fragments are harmless', async ({ page }) => {
    await page.goto('about:blank'); await page.goto(origin + route + '#support-community-step-03', { waitUntil: 'networkidle' });
    const section = page.locator('#support-community-step-03'); await expect(section).toBeFocused();
  ...
  test('chapter actions and related destinations navigate to existing guidance, not commands', async ({ page }) => {
    const actions = page.locator('.lgo-support-community-action'); await expect(actions).toHaveCount(4);
    const routes = ['/support/help', '/support/safety', '/community', '/status'];
    ...
    await actions.last().click(); await expect(page).toHaveURL(origin + '/status');
  });
  test('source paper typography and actions remain legible at 320px with visible focus', async ({ page }) => {
    ...
    for (const link of await guide.locator('.lgo-article-section-navigation a,.lgo-support-community-action').all()) expect((await link.boundingBox())!.height).toBeGreaterThanOrEqual(44);
    const action = guide.locator('.lgo-support-community-action').first(); await action.focus(); await expect(action).toBeFocused();
  ...
  test('reading has no submission saved progress access award or gameplay surrogate', async ({ page }) => {
    const guide = page.locator('.lgo-support-community-guide'); await expect(guide).toBeVisible();
    await expect(guide).toContainText('Hướng dẫn đọc và chuẩn bị phản hồi, không phải nơi gửi yêu cầu hỗ trợ'); await expect(guide).toContainText('NO_ACCEPTED_BACKEND_CONTRACT');
    ...
    await page.getByRole('navigation', { name: 'Mục lục hỗ trợ và cộng đồng' }).locator('a').first().click();
    await page.locator('#support-community-step-01').getByRole('link', { name: /Phần tiếp/ }).click();
    expect(requests).toEqual([]); await expect(page.locator('main h1')).toHaveCount(1);
    await expect(guide.locator('.lgo-article-instruction').nth(1)).toContainText('không gửi mật khẩu, token hoặc dữ liệu nhạy cảm');
    await expect(guide.getByRole('button', {name: /Gửi yêu cầu|Mở ticket|Vào chat|Đăng nhập/i})).toHaveCount(0);
