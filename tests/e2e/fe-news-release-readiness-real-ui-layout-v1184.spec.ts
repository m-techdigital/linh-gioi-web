...
test.describe("news release readiness real UI layout v1.184", () => {
  test("/news/release-readiness-hub-polish-started renders compact Vietnamese release readiness article flow", async ({ page, isMobile }) => {
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Release readiness hub rõ ràng hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích release readiness hub" })).toBeVisible();
