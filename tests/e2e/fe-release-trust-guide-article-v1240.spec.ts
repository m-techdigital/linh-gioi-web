...
const route = '/guides/release-trust-and-checksum-guide';
const entry = contentEntries.find(item => item.slug === 'release-trust-and-checksum-guide')!;
const steps = guideDetailSteps.filter(item => item.slug === entry.slug);

test.describe('release trust is an authored guide, not artifact certification v1.240', () => {
  ...
  test('four full article chapters and responsive contents replace compact proof boards', async ({ page, isMobile }) => {
    const guide = page.locator('.lgo-release-trust-guide');
    ...
    await page.screenshot({ path: test.info().outputPath('release-trust-article.png'), fullPage: true });
  });
  test('original summary body and all four instruction result boundary records are exact', async ({ page }) => {
    const guide = page.locator('.lgo-release-trust-guide'); await expect(guide).toBeVisible();
  ...
  test('native contents next previous and browser history follow the real chapter IDs', async ({ page }) => {
    const contents = page.getByRole('navigation', { name: 'Mục lục tin cậy phát hành' });
    const links = contents.locator('a'); await expect(links).toHaveCount(4);
    await links.nth(1).focus(); await page.keyboard.press('Enter'); await expect(page.locator('#release-trust-step-02')).toBeFocused();
    await page.locator('#release-trust-step-02').getByRole('link', { name: /Phần tiếp/ }).click();
    await expect(page).toHaveURL(origin + route + '#release-trust-step-03'); await expect(page.locator('#release-trust-step-03')).toBeFocused();
    await page.goBack(); await expect(page).toHaveURL(origin + route + '#release-trust-step-02');
    await page.goForward(); await expect(page).toHaveURL(origin + route + '#release-trust-step-03');
    await page.locator('#release-trust-step-03').getByRole('link', { name: 'Về mục lục', exact: true }).click();
    await expect(page.locator('#release-trust-contents')).toBeFocused();
    const summary = page.locator('#release-trust-contents summary'); await summary.focus(); await page.keyboard.press('Space');
  ...
  test('cold direct URLs land visibly at the requested chapter and malformed fragments are harmless', async ({ page }) => {
    await page.goto('about:blank'); await page.goto(origin + route + '#release-trust-step-03', { waitUntil: 'networkidle' });
    const section = page.locator('#release-trust-step-03'); await expect(section).toBeFocused();
  ...
  test('chapter actions and related destinations navigate to existing guidance, not commands', async ({ page }) => {
    const actions = page.locator('.lgo-release-trust-action'); await expect(actions).toHaveCount(4);
    const routes = ['/download', '/download/trust', '/release', '/status'];
  ...
  test('source paper typography and actions remain legible at 320px with visible focus', async ({ page }) => {
    ...
    for (const link of await guide.locator('.lgo-article-section-navigation a,.lgo-release-trust-action').all()) expect((await link.boundingBox())!.height).toBeGreaterThanOrEqual(44);
    const action = guide.locator('.lgo-release-trust-action').first(); await action.focus(); await expect(action).toBeFocused();
  ...
  test('reading has no submission saved progress access award or gameplay surrogate', async ({ page }) => {
    const guide = page.locator('.lgo-release-trust-guide'); await expect(guide).toBeVisible();
    await expect(guide).toContainText('Hướng dẫn đối chiếu, không phải chứng nhận gói build'); await expect(guide).toContainText('NO_ACCEPTED_BACKEND_CONTRACT');
    ...
    await page.getByRole('navigation', { name: 'Mục lục tin cậy phát hành' }).locator('a').first().click();
    await page.locator('#release-trust-step-01').getByRole('link', { name: /Phần tiếp/ }).click();
    expect(requests).toEqual([]); await expect(page.locator('main h1')).toHaveCount(1);
    await expect(guide.locator('.lgo-article-instruction').nth(1)).toContainText('SHA256');
    await expect(guide.locator('.lgo-article-instruction').nth(1)).toContainText('không dùng hash mẫu');
