...
test.describe("news closed tester pack real UI layout v1.183", () => {
  test("/news/closed-tester-information-pack-started renders compact Vietnamese closed tester information article flow", async ({ page, isMobile }) => {
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Gói thông tin closed tester đã được chuẩn bị" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích closed tester information pack" })).toBeVisible();
