// v1.191 coverage: /news/accessibility-readability-polish-started must render compact Vietnamese accessibility readability article with detail cards, not a body-only accessibility fixture.
...
test.describe("news accessibility readability real UI layout v1.191", () => {
  test("/news/accessibility-readability-polish-started renders compact Vietnamese accessibility readability article flow", async ({ page, isMobile }) => {
    await page.goto(`${web}/news/accessibility-readability-polish-started`);

    await expect(page.getByRole("heading", { level: 1, name: "Dễ đọc và dễ tiếp cận hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích khả năng tiếp cận và dễ đọc" })).toBeVisible();
    ...
    expect(mainText).not.toMatch(/Bài viết giải thích nội dung public|formal WCAG audit|legal compliance|production support|backend integration|ticket live/i);
    ...
    await page.screenshot({ path: `/tmp/news-accessibility-readability-${isMobile ? "mobile" : "desktop"}-v1191.png`, fullPage: true });
  });
});
