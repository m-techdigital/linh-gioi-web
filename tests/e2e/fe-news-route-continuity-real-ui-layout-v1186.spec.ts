...
test.describe("news route continuity real UI layout v1.186", () => {
  test("/news/route-continuity-conversion-polish-started renders compact Vietnamese route continuity article flow", async ({ page, isMobile }) => {
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Route continuity và bước tiếp an toàn rõ hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích route continuity và conversion-safe" })).toBeVisible();
