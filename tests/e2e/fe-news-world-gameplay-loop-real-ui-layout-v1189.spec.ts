// v1.189 coverage: /news/world-gameplay-loop-depth-started must render as compact Vietnamese world gameplay loop article, not English gameplay boundary copy.
...
test.describe("news world gameplay loop real UI layout v1.189", () => {
  test("/news/world-gameplay-loop-depth-started renders compact Vietnamese world gameplay loop article flow", async ({ page, isMobile }) => {
    await page.goto(`${web}/news/world-gameplay-loop-depth-started`);

    await expect(page.getByRole("heading", { level: 1, name: "World loop và kỳ vọng gameplay rõ hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích world loop và boundary gameplay" })).toBeVisible();
    ...
    expect(mainText).not.toMatch(/guided world loop explanation|No combat damage|HP|loot|inventory\/economy|quest DB|live world server|No fake download CTA|production auth|backend recommendation engine/i);
    ...
    await page.screenshot({ path: `/tmp/news-world-gameplay-loop-${isMobile ? "mobile" : "desktop"}-v1189.png`, fullPage: true });
  });
});
