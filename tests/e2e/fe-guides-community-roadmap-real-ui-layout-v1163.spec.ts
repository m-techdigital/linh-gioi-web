// v1.163 coverage: /guides/community-roadmap-onboarding-guide prioritizes rendered browser UI layout and shared guide-flow Base First CSS.
...
async function collect(page: Page): Promise<Metrics> {
  return page.evaluate(() => {
    ...
    const hero = rect(".lgo-community-roadmap-hero-card");
    ...
    return {
      ...
      firstFlowText: [
        document.querySelector(".lgo-community-roadmap-hero-card")?.textContent ?? "",
...
test.describe("community roadmap guide real UI layout v1.163", () => {
  test("/guides/community-roadmap-onboarding-guide renders roadmap/community expectation flow before generic CTAs", async ({ page, isMobile }, testInfo) => {
    await page.goto(`${web}/guides/community-roadmap-onboarding-guide`);

    await expect(page.getByRole("heading", { level: 1, name: "Lộ trình cộng đồng đúng kỳ vọng" })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Các bước guide có kết quả mong đợi và phạm vi tạm khóa" })).toBeVisible();
    await expect(page.locator(".lgo-status-badge", { hasText: "Lộ trình cộng đồng" }).first()).toBeVisible();

    const metrics = await collect(page);
    expect(metrics.firstHeadings[0]).toEqual({ tag: "h1", text: "Lộ trình cộng đồng đúng kỳ vọng" });
    expect(metrics.firstFlowText).toContain("Trạng thái chơi");
    expect(metrics.firstFlowText).toContain("Tin cậy tải game");
    expect(metrics.firstFlowText).toContain("mốc quyết định");
    expect(metrics.firstFlowText, "generic guide fallback must not drive the current page layout").not.toMatch(/WEB v1\.155|Guide tĩnh · chưa có hệ thống wiki|public status|roadmap decision gates|static guidance|No public game download artifact|account portal/i);
    ...
    const statusLink = page.locator(".lgo-community-roadmap-hero-card").getByRole("link", { name: "Trạng thái chơi" });
    ...
    if (isMobile) {
      ...
      expect(metrics.firstStepTop, "mobile first roadmap step appears in first fold").toBeLessThanOrEqual(790);
      expect(metrics.worldCtaTop, "mobile generic CTA starts after roadmap steps without a raw gap").toBeLessThanOrEqual(1310);
      ...
      expect(metrics.routeCtaTop, "desktop route CTA stays early").toBeLessThanOrEqual(1150);
    ...
    const screenshotPath = `/tmp/guides-community-roadmap-${isMobile ? "mobile" : "desktop"}-v1163.png`;
    await page.screenshot({ path: screenshotPath, fullPage: true });
    await testInfo.attach(`guides-community-roadmap-${isMobile ? "mobile" : "desktop"}-v1163`, {
      path: screenshotPath,
