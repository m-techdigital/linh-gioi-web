...
const slug='news-guide-detail-pages-started',route=`/news/${slug}`;
...
test.describe('source-complete news and guide detail article v1.258',()=>{
 ...
 test('reading layout and responsive contents replace compact proof cards',async({page,isMobile})=>{
  ...
  await page.screenshot({path:test.info().outputPath('news-detail-reading-article.png'),fullPage:true});
 });
 ...
 test('contents previous next and Back Forward move between the real sections',async({page})=>{
  const links=page.getByRole('navigation',{name:'Mục lục bài viết tin và hướng dẫn'}).getByRole('link');await expect(links).toHaveCount(2);
 ...
 test('reduced motion uses immediate native section arrival rather than smooth scrolling',async({page})=>{
  ...
  await page.getByRole('navigation',{name:'Mục lục bài viết tin và hướng dẫn'}).getByRole('link').last().click();
 ...
 test('resizing the same article retains complete chapters and a single coherent contents navigation',async({page})=>{
  await page.emulateMedia({reducedMotion:'reduce'});
  const nav=page.getByRole('navigation',{name:'Mục lục bài viết tin và hướng dẫn'});
