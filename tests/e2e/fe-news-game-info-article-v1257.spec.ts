...
const slug='public-game-info-depth-started',route=`/news/${slug}`;
...
test.describe('source-complete public game information article v1.257',()=>{
 ...
 test('reading layout and responsive contents replace compact proof cards',async({page,isMobile})=>{
  ...
  await page.screenshot({path:test.info().outputPath('news-game-info-article.png'),fullPage:true});
 });
 ...
 test('contents previous next and Back Forward move between the real sections',async({page})=>{
  const links=page.getByRole('navigation',{name:'Mục lục bài viết thông tin game'}).getByRole('link');await expect(links).toHaveCount(2);
 ...
 test('reduced motion uses immediate native section arrival rather than smooth scrolling',async({page})=>{
  ...
  await page.getByRole('navigation',{name:'Mục lục bài viết thông tin game'}).getByRole('link').last().click();
 ...
 test('resizing the same article retains complete chapters and a single coherent contents navigation',async({page})=>{
  await page.emulateMedia({reducedMotion:'reduce'});
  const nav=page.getByRole('navigation',{name:'Mục lục bài viết thông tin game'});
 ...
 test('expanded text spacing does not clip authored text or hide reading links',async({page})=>{
  ...
  await root.locator('.lgo-news-article-action').last().focus();await page.keyboard.press('Enter');await expect(page).toHaveURL(origin+'/status');
 });
