// v1.180 coverage: /news/public-game-info-depth-started must render as compact Vietnamese game-info article detail, not a raw WEB v1.8 fixture body.
...
test.describe("news public game info real UI layout v1.180", () => {
  test("/news/public-game-info-depth-started renders compact Vietnamese game-info article detail flow", async ({ page, isMobile }) => {
    await page.goto(`${web}/news/public-game-info-depth-started`);

    await expect(page.getByRole("heading", { level: 1, name: "Thông tin game public được làm sâu hơn" })).toBeVisible();
    await expect(page.getByText("Bài viết công khai", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Bài viết giải thích thông tin game public" })).toBeVisible();
    ...
    expect(mainText).not.toMatch(/WEB v1\.8 focuses on the actual public website product|world story chapters|beginner guide steps|runtime\/browser checks remain guardrails only|claim hệ thống production|WEB v1\.9 article detail|Boundary:|PROVISIONAL_WEB_FIXTURE|Detail section|This public content is/i);
    ...
    await page.screenshot({ path: `/tmp/news-public-game-info-${isMobile ? "mobile" : "desktop"}-v1180.png`, fullPage: true });
  });
});
