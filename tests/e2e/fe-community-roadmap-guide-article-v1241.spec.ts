...
const route = '/guides/community-roadmap-onboarding-guide';
const entry = contentEntries.find(item => item.slug === 'community-roadmap-onboarding-guide')!;
const steps = guideDetailSteps.filter(item => item.slug === entry.slug);

test.describe('community roadmap is reading guidance, not an invitation or schedule v1.241', () => {
  ...
  test('four full article chapters and responsive contents replace compact proof boards', async ({ page, isMobile }) => {
    const guide = page.locator('.lgo-community-roadmap-guide');
    ...
    await page.screenshot({ path: test.info().outputPath('community-roadmap-article.png'), fullPage: true });
  });
  test('original summary body and all four instruction result boundary records are exact', async ({ page }) => {
    const guide = page.locator('.lgo-community-roadmap-guide'); await expect(guide).toBeVisible();
  ...
  test('native contents next previous and browser history follow the real chapter IDs', async ({ page }) => {
    const contents = page.getByRole('navigation', { name: 'Mục lục lộ trình cộng đồng' });
    const links = contents.locator('a'); await expect(links).toHaveCount(4);
    await links.nth(1).focus(); await page.keyboard.press('Enter'); await expect(page.locator('#community-roadmap-step-02')).toBeFocused();
    await page.locator('#community-roadmap-step-02').getByRole('link', { name: /Phần tiếp/ }).click();
    await expect(page).toHaveURL(origin + route + '#community-roadmap-step-03'); await expect(page.locator('#community-roadmap-step-03')).toBeFocused();
    await page.goBack(); await expect(page).toHaveURL(origin + route + '#community-roadmap-step-02');
    await page.goForward(); await expect(page).toHaveURL(origin + route + '#community-roadmap-step-03');
    await page.locator('#community-roadmap-step-03').getByRole('link', { name: 'Về mục lục', exact: true }).click();
    await expect(page.locator('#community-roadmap-contents')).toBeFocused();
    const summary = page.locator('#community-roadmap-contents summary'); await summary.focus(); await page.keyboard.press('Space');
  ...
  test('cold direct URLs land visibly at the requested chapter and malformed fragments are harmless', async ({ page }) => {
    await page.goto('about:blank'); await page.goto(origin + route + '#community-roadmap-step-03', { waitUntil: 'networkidle' });
    const section = page.locator('#community-roadmap-step-03'); await expect(section).toBeFocused();
  ...
  test('chapter actions and related destinations navigate to existing guidance, not commands', async ({ page }) => {
    const actions = page.locator('.lgo-community-roadmap-action'); await expect(actions).toHaveCount(4);
    const routes = ['/status', '/roadmap', '/support/safety', '/community'];
    ...
    await expect(actions.nth(1)).toHaveAttribute('href','/roadmap');
    await actions.last().click(); await expect(page).toHaveURL(origin + '/community');
  });
  test('source paper typography and actions remain legible at 320px with visible focus', async ({ page }) => {
    ...
    for (const link of await guide.locator('.lgo-article-section-navigation a,.lgo-community-roadmap-action').all()) expect((await link.boundingBox())!.height).toBeGreaterThanOrEqual(44);
    const action = guide.locator('.lgo-community-roadmap-action').first(); await action.focus(); await expect(action).toBeFocused();
  ...
  test('reading has no submission saved progress access award or gameplay surrogate', async ({ page }) => {
    const guide = page.locator('.lgo-community-roadmap-guide'); await expect(guide).toBeVisible();
    await expect(guide).toContainText('Bài hướng dẫn không mở đăng ký, không cấp quyền kiểm thử'); await expect(guide).toContainText('NO_ACCEPTED_BACKEND_CONTRACT');
    ...
    await page.getByRole('navigation', { name: 'Mục lục lộ trình cộng đồng' }).locator('a').first().click();
    await page.locator('#community-roadmap-step-01').getByRole('link', { name: /Phần tiếp/ }).click();
    expect(requests).toEqual([]); await expect(page.locator('main h1')).toHaveCount(1);
    await expect(guide.locator('.lgo-article-boundary').last()).toContainText('Chưa có lịch test được duyệt');
