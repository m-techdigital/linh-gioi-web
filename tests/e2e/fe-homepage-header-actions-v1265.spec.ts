...
test.describe('homepage header and main actions v1.265 revision3',()=>{
 ...
 test('header reuses the source brush brand and explicitly marks the homepage as current',async({page})=>{
  const header=page.locator('header.lgo-site-header'),brand=header.getByRole('link',{name:'Linh Giới Online — Trang chủ',exact:true});await expect(brand.locator('.lgo-art-wordmark')).toHaveCount(1);await expect(brand.locator('img')).toHaveAttribute('src','/game-art/marketing/wordmark-brush.png');await expect(brand).toHaveAttribute('href','/');
