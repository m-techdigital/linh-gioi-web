// v1.88 coverage: trust/onboarding routes start main content with the page h1 before CTA sections.
import { test, expect, type Page } from "@playwright/test";

const web = process.env.LGO_WEB_URL ?? "http://127.0.0.1:3000";
const routes = [
  { path: "/download/trust", title: "Download trust / checksum / provenance" },
  { path: "/roadmap", title: "Roadmap phát triển web" },
  { path: "/community/onboarding", title: "Community / roadmap onboarding" },
];
...
test.describe("public trust heading priority", () => {
  for (const route of routes) {
    test(`${route.path} starts trust content with the page h1`, async ({ page, isMobile }) => {
